...
public class RefTreeByIdEncoder extends RefTreeEncoder {
  ...
  @Override
  protected String getBaseRefTarget(int branchPos, Segment<Item> match,
      MultiXPath xp) {
    int steps = -1, depth = 0;
    int offset = branchPos - match.getPosition();
    int basePos = match.getOffset() + offset;
    //Log.debug("<<<Start identify scan at pos "+basePos);
    // Scan backwards to a parent tag with id. At the same time, we count the
    // position in the child list of the parent.
    // NOTE: We only scan up to the parent, i.e. the tag <x> in 
    // <t id="i"><t><x> won't get an id. (We could use a recursive scheme here, 
    // to get something like i.0.0)
    while ( basePos > 0 && depth >= 0 &&
        (depth> 0 || !Item.isStartTag( base.get(basePos) ) ) ) {
    ...
    String baseId = null;
    if (basePos > 0 && ((StartTag) base.get(basePos)).getAttribute(
        ID_ATTR ) != null) {
        baseId = ((StartTag) base.get(basePos)).getAttribute(
            ID_ATTR).getValue().toString();
    }
    if ( baseId == null )
      return UNKNOWN_ID_PREFIX+(branchPos - match.getPosition());
    if (steps >= 0)
      return baseId+"."+steps;
    return baseId;
  }
  ...
  @Override
  public void encodeDiff(List<Item> base, List<Item> doc, List<Segment<Item>> matches, List<Item> preamble, OutputStream out) throws IOException {
    this.base = base;
