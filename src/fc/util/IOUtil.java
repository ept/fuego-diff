...
public class IOUtil {
    ...
    /**
     * Stream close convenience method. If the given stream is not <code>null</code>, it is closed.
     * If an exception occurs due to the close, this is logged as a fatal error.
     * @param in
     *            stream to close
     */
    ...
    public static File getTempName(File dir, String prefix, String suffix) throws IOException {
        int left = 10;
        prefix = Util.isEmpty(prefix) ? "" : prefix + "-";
        for (File f; (f = new File(dir, prefix +
                                        Integer.toHexString(((rnd.nextInt() & 0xffff) | 0x1000)) +
                                        suffix)) != null; left--) {
            if (left <= 0) throw new IOException("Cannot make temporary name");
    ...
    public static File createTempFile(File f) throws IOException {
        ...
        if (!nf.createNewFile()) throw new IOException("Cannot create " + f);
        return nf;
    ...
    public static File createTempFile(File dir, String prefix, String suffix) throws IOException {
        ...
        if (!f.createNewFile()) throw new IOException("Cannot create " + f);
        return f;
    }
    ...
    public static boolean replace(File srcFile, File targetFile) throws IOException {
    ...
    public static File backup(File f, File tmp) throws IOException {
        ...
        if (!f.renameTo(tmp)) {
            if (canMoveOver && !tmp.delete()) throw new IOException("Cannot delete " + tmp);
    ...
    private static boolean determineMoveOver(File tmp) throws IOException {
        ...
        if (canMoveOver == null) {
            ...
            try {
                ...
                if (!tmp2.createNewFile()) throw new IOException("Cannot create " + tmp);
                ...
                if (canMoveOver && !tmp2.renameTo(origTmp)) {
                    ...
                    Log.fatal("Cannot move potentially valuable data in " + tmp2 + "back to file " +
                              origTmp);
                ...
                if (tmp2.exists() && !tmp2.delete()) Log.fatal("Cannot delete temp file" + tmp2);
     ...
     */
    public static int copyStream(InputStream src, OutputStream dest, long maxLeft)
            throws IOException {
    ...
    /**
     * Delete directory tree. Deletes the directory tree rooted at <code>f</code> including
     * <code>f</code>.
     * @param f
    ...
    /**
     * Delete directory tree. Deletes the directory tree rooted at <code>f</code>. The directory
     * <code>f</code> itself is deleted if <code>delRoot</code> is <code>true</code>.
     * @param f
     *            root of tree to delete
     * @param delRoot
     *            set to <code>true</code> to delete root.
     * @throws IOException
     *             if an I/O error occurs
     */
     ...
     *            root of tree to delete
     * @param fi
     *            filter that an entry must pass in order to be deleted ( <code>null</code> means
     *            all pass)
     * @param delRoot
     *            set to <code>true</code> to delete root.
     * @throws IOException
