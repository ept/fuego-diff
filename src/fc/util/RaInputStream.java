...
public class RaInputStream extends InputStream implements SeekableInputStream {
    ...
    public int read() throws IOException {
        ...
        left--;
        // Log.debug("1-byte read returning char "+Debug.toPrintable(buffer,
        // offset, 1)+" at "+offset+" left is = "+left);
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        if (left > 0) {
             ...
             * (currentFilePos-left)+", requested="+len, Debug.toPrintable(b,off,toCopy));
             */
            left -= toCopy;
            offset += toCopy;
         ...
         * currentFilePos+", requested="+len, Debug.toPrintable(b,off,read));
         */
