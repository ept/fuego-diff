...
public class GlMatcher<E> {
    ...
    /*
     * Experimental support for aligning on boundaries. Does not work well in all cases. Consder the
     * following (bounds on .) base="bar. There is nothing like foobar." matched=ins<"bar. There">,
     * scanning for < is nothing... > We match the scan to <There is nothing ...> -> "There" occurs
     * twice in the match.
     */
    ...
    public List<Segment<E>> match(List<E> base, List<E> doc, int[] sizes) {
        ...
        // Log.log("Final list\n"+matchList,Log.INFO);
        /*
         * Log.log("Matching took " + (timeS - timeB) + "msec, simplify " + (timeU - timeS) +
         * "msec, updatify "+ (timeE - timeU) + "msec.", Log.INFO);
         */
    ...
    public void updatify(List<Segment<E>> ml, List<Segment<E>> deletia, int baseLen) {
        ...
        for (int i = 0; i < ml.size(); i++) {
            ...
            if (current.getOp() == INSERT && (prev == null || prev.getOp() == COPY) &&
                (next == null || next.getOp() == COPY)) {
                int rqstart = prev != null ? prev.getOffset() + prev.getLength() : 0;
                int rqend = next != null ? next.getOffset() : baseLen;
                if (rqend <= rqstart) continue; // Copys out of order in src
                // Scan deletia; FIXME: use lookup(ix)->del yes/no for max
                // efficiency
                for (ListIterator<Segment<E>> j = deletia.listIterator(); j.hasNext();) {
                    Segment<E> del = j.next();
                    if (del.getOffset() > rqstart) break; // Done, if we assume order by start
                    if (del.getOffset() < rqstart) continue;
                    if (del.getOffset() + del.getLength() < rqend) continue; // Another move chunk
                    // m3 in between in
                    // src:
                    // m1.d.m3.m2
                    ...
                    // This should not happen; e.g. mmmmmddnnnn (m,n matched
                    // tokens)
                    // and deletia. length != 2
                    // Log.log("Deletia overflows its space",Log.ASSERTFAILED);
                    j.remove(); // No longer deleted
                    ml.set(i, Segment.createUpdate(del.getOffset(), del.getLength(),
                                                   current.getInsert(), current.getPosition()));
                }
            }
        }
    }
    ...
    protected void findChunks(List<Segment<E>> currentOps, List<Segment<E>> base, int chunkSize,
                              int minSize) {
        int firstRegion = 0;
        int scanpos = -1; // -1 indicates that we need a new chunk from doclist
        Segment<E> m = null;
        Segment<E> collatedIns = null; // Used to collect multiple emitted
        // sequential
        // ins ops into one single ins op. See note about collate above
        Segment<E> match = null;
        int collatedLen = -1;
        if (base.size() == 0) return; // Nothing left in base to match!
        for (ListIterator<Segment<E>> i = currentOps.listIterator(); scanpos != -1 || i.hasNext();) {
            if (scanpos == -1) {
                m = i.next();
                if (m.getOp() == COPY) {
                ...
                if (m.getOp() == INSERT && m.getLength() < chunkSize) continue;
            ...
            int[] offlen = findChunkInRegions(scanpos, m.getInsert(), base, firstRegion, chunkSize,
                                              minSize);
            ...
            if (offlen == null) {
                ...
                /*
                 * List<E> realins = m.getInsert().subList(scanpos, scanpos + inssize);
                 */
                if (collatedIns == null) {
                    // NOTE: This copy op is just a placeholder for scanpos; we
                    // emit it
                    // as insert when collation is flushed
                    collatedIns = Segment.<E> createCopy(scanpos, -1, scanpos);
                    collatedLen = inssize;
                } else {
                    // Should currentOps <ins> should always append
                    // back-to-back; check this
                ...
                match = Segment.<E> createCopy(offlen[0], offlen[1], m.getPosition() + scanpos);
                if ((scanpos + offlen[1]) < m.getInsert().size()) scanpos += offlen[1];
                else scanpos = -1; // need next
    ...
    protected int[] findChunkInRegions(int scanpos, List<E> chunkToMatch,
        ...
        // Log.log("Looking for " + chunkToMatch.subList(scanpos,
        // chunkToMatch.size()),
        // Log.INFO);
        loop: for (int ofi = firstRegion; ofi < firstRegion + baseRegions.size(); ofi++) {
            int i = (baseRegions.size() + bfly + firstRegion) % baseRegions.size();
            if (bfly <= 0) // 0,1,-1,2,-2,3,-3,...
            bfly = -bfly + 1;
            else bfly = -bfly;
            if (bfly > maxbfly || bfly < -maxbfly) return null;
            Segment<E> region = baseRegions.get(i);
            // Log.log("Looking into ix "+i+":"+region,Log.INFO);
            // assuming off 0 is start of region...; offlen coordinates in
            // region,
            // corresponding offset in chunkToMatch is ***offlen[0]+scanpos***
            int[] offlen = findChunk(scanpos, chunkToMatch, region.getInsert(), chunkSize, minSize);
            if (offlen != null) {
                // Log.log("Found match @ "+offlen[0]+", len "+offlen[1]
                // ,Log.INFO);
                ...
                if (tokenBoundaries != null) {
                    ...
                    // Determine low
                    {
                        ...
                        if (low < 0) {
                            // in between, se up low so we're between low and
                            // low+1
                            ...
                            start = origStart <= mid && tokenBoundaries[low] >= regionMin ? tokenBoundaries[low]
                                    : tokenBoundaries[low + 1];

                        } else start = origStart;
                    }
                    // Now, do it fo hi
                    {
                        ...
                        if (hi < 0) {
                            ...
                            end = origEnd > mid && tokenBoundaries[hi + 1] <= regionMax ? tokenBoundaries[hi + 1]
                                    : tokenBoundaries[hi];
                        } else end = origEnd;
                    }
                    if (end <= start) continue loop; // Aligned one turned out to beinvalid;
                    // case end < start
                ...
                if (offlen[0] > 0) {
                    Segment<E> pre = Segment.createIns(region.getOffset(),
                                                       region.getInsert().subList(0, offlen[0]),
                                                       region.getOffset());
                ...
                if (offlen[0] + offlen[1] < region.getLength()) {
                    ...
                    Segment<E> post = Segment.createIns(
                                                        start + region.getOffset(),
                                                        region.getInsert().subList(
                                                                                   start,
                                                                                   region.getInsert().size()),
                                                        start + region.getOffset());
    ...
    protected int[] findChunk(int scanpos, List<E> chunk, List<E> baseRegion, int chunkSize,
                              int minSize) {
        // Log.log("findChunk: ",Log.INFO, new int[]
        // {scanpos,chunk.length,baseRegion.length,maxsize});
        int len = Math.min(chunkSize, chunk.size() - scanpos);
        if (baseRegion.size() < len || len < minSize) return null; // Doesn't fit
        ...
        for (int i = 0; i + len <= baseRegion.size(); i++) {
            if (froll[0] == broll[0] && froll[1] == broll[1]) {
                ...
                if (j == len) {
                    // try expanding left
                    int maxmore = Math.min(chunk.size() - len - scanpos, baseRegion.size() - i -
                                                                         len);
                    int extra = 0;
                    while (extra < maxmore &&
                           baseRegion.get(i + len + extra).equals(chunk.get(scanpos + len + extra)))
                        extra++;
                    /*
                     * if( extra < maxmore ) { Log.log("Stopped due to 1:"+baseRegion.get(i + len +
                     * extra),Log.INFO); Log.log("Stopped due to 2:"+chunk.get(scanpos + len +
                     * extra),Log.INFO); }
                     */
                 ...
                 * " event@base="+baseRegion.get(i+j)+" equals="
                 * +chunk.get(scanpos+j).equals(baseRegion.get(i+j))+ "\n"+chunk
                 * .subList(scanpos,scanpos+len)+" to\n"+baseRegion .subList(i,i+len),Log.INFO);
                 */
                falseHashMatches++;
            }
            // Expunge this byte, and chew next, unless on last (test-only) lap
            if (i + len < baseRegion.size())
                updateroll(broll, baseRegion.get(i), baseRegion.get(i + len), i, i + len);
        }
    ...
    protected final void updateroll(short[] state, E outT, E inT, int k, int l) {
        ...
        /*
         * state[0]+=in-out; short tmp =(short) (((short) (l-k))out); short tmp2 = (short)
         * (state[1]-tmp); state[1]=(short) (tmp2+state[0]);
         */
        state[0] += in - out;
