    ...
    public void start() throws IOException {
        ...
        for (Iterator i = dirs.entrySet().iterator(); i.hasNext();) {
            ...
            for (int test = 0; test < Integer.MAX_VALUE; test++) {
                ...
                if (p.getProperty(pfx + "basec") == null && p.getProperty(pfx + "base") == null)
                    break;
        ...
        if (fail > 0) Assert.fail("Test failed due to new failures.");
    }


    protected File getFile(Properties p, String prop, File propRoot, File defaultFile,
                           String sysprop) {
        String name = p.getProperty(prop);
        File f = name == null ? defaultFile : new File(propRoot, name.trim());
        if (f != null && sysprop != null) System.setProperty(sysprop, f.toString());
        return f;
    }


    protected void doDiff(File basef, File newf, File deltaf) throws IOException {
        // Log.log("Test", Log.INFO,new String[]
        // {root.toString(),basen,newn,deltan,newcn,basecn});
        String reporter = getSetProperty("reporter", null);
        long now = System.currentTimeMillis();
        try {
            exec(getSetProperty("differ", "!fc.xml.diff.Diff"), new String[] { basef.toString(),
                                                                              newf.toString(),
                                                                              deltaf.toString() },
                 true, getSetProperty("logfile", null),
    ...
    public static void scanDataSets(Map dirs, File root, String df, String ff) {
        ...
        if (root.isDirectory()) {
            ...
            for (int i = 0; i < e.length; i++) {
                if (e[i].getPath().matches(df)) scanDataSets(dirs, e[i], df, ff);
            }
