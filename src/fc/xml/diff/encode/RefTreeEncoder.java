...
public class RefTreeEncoder implements DiffEncoder {
    ...
    protected Boolean encodeDiff(ListOfMatchedEvents mr, ItemTarget out, boolean parentChanged,
                                 LinkedList<RefItem> tagStack, MultiXPath xp, String[] topPath,
                                 List<Item> base) throws IOException {
        // Log.debug("peek()="+mr.peek()+", topPath="+topPath[0]+", pos="+
        // mr.getCurrentPosition());
        Item e = mr.peek();
        if (Item.isEndTag(e)) { return null; // End of childlist
        }
        mr.next(); // BUGFIX070619-1: Don't do assert e == mr.next();, -da wont
        // work
        ...
        if (!changed) {
            ...
            tagStack.addLast(RefItem.makeStartItem(
                                                   isText ? new TreeReference(
                                                                              StringKey.createKey(thisPath))
                                                           : new NodeReference(
                                                                               StringKey.createKey(thisPath)),
                                                   null));
        } else {
        ...
        for (Boolean childchange = null; !isText &&
                                         (childchange = encodeDiff(mr, out, changed |
                                                                            childrenChanged,
                                                                   tagStack, xp, topPath, base)) != null;) {
            Item nexte = mr.currentItem();
            if (nexte != null && mr.getCurrentTag() != matchTag)
            // NOTE,FIXME-070620-1: Detects back-to-back CPY ops as a
                // discontinuity
                orderChanged = true;
        ...
        if (isTag) {
            ...
            // We require that the end tag belongs to the same match area for
            // this.
            // A stricter test would be to test that the last matched child item
            // was
            // at the end of the match area. However, I do not think this makes
            // much
            // of a difference
            if (!orderChanged && e != null && mr.getCurrentTag() != matchTag) orderChanged = true;
        }
        ...
        if (!childrenChanged && !changed && !orderChanged) {
            ...
            // The size()=1 case below is when a tentative open-ref-node is on
            // the
            // stack (which will be converted to a ref-tree node and emitted)
            assert (tagStack.size() == (!changed && !childrenChanged && !orderChanged ? 1 : 0));
        }
        if (!changed) {
            if (!childrenChanged) {
                tagStack.removeLast(); // Ref will never be needed
                ...
                RefItem ri = RefItem.makeEndItem(new NodeReference(StringKey.createKey(thisPath)));
                out.append(ri);
            }
        } else if (isTag) out.append(e); // EE (only with elements)
        assert (!changed || tagStack.size() == 0);
        topPath[0] = thisPath; // NOTE: Keep close to return to ensure correct
        // topPath
    ...
    protected String getBaseRefTarget(int branchPos, Segment<Item> match, MultiXPath xp) {
        ...
        // The _xp2 code will just make a new NumXPath, and seek to pos from
        // start
        // NumericXPath __xp2 = new NumericXPath(
        // xp.paths.get(xp.paths.firstKey()).es );
    ...
    public static class MultiXPath {
        ...
        private SortedMap<Integer, NumericXPath> paths = new TreeMap<Integer, NumericXPath>();
    ...
    private static boolean isTextLike(Item i) {
        ...
        int type = i.getType();
        return type == Item.TEXT || type == Item.COMMENT || type == Item.ENTITY_REF ||
               type == Item.PI;
    }
    ...
    public static class NumericXPath {
            ...
            for (; pos < target; pos++) {
                ...
                switch (type) {
                    ...
                    case Item.START_DOCUMENT:
                        // FIXME-20061113-1:Item types not yet in XAS2. Fix
                        // commented out below
        ...
        public String toString() {
            ...
            if (lastWasText) sb.append('/').append(counters[len] - 1);
            return sb.toString();
        }
        ...
        public static void test(List<Item> es) {
            ...
            for (int i = 0; es.get(i) != null; i++) {
                xp.moveto(i + 1);
                System.out.println(StringUtil.format(String.valueOf(i), -5) +
