...
public class XmlDiffEncoder extends RefTreeEncoder {
    ...
    /**
     * ReferenceEs to Diff es. The input es ids are assumed to be absolute XPaths of the form
     * /n1/n2.../nm. The XPaths in the output are either full paths, or relative to the parent.
     */
    ...
    public static class RefEStoDiffES implements ItemTransform {
        ...
        public void append(Item ev) throws IOException {
            if (state == 1) {
                if (ev.getType() == RefTreeItem.TREE_REFERENCE &&
                    appends(prevRef.getTarget().toString(), ((RefItem) ev).getTarget().toString())) {
                    run++;
                    ...
                    st.addAttribute(fc.xml.xmlr.tdm.Diff.DIFF_CPYRUN_ATTR, String.valueOf(run));
                ...
                case RefNodeItem.NODE_REFERENCE:
                    ...
                    if (!ni.isEndTag()) {
                        ...
                        queue.add(RefItem.makeStartItem(
                                                        new NodeReference(
                                                                          StringKey.createKey(getRelativePath(
                                                                                                              parentPaths.peek(),
                                                                                                              ni.getTarget().toString()))),
                                                        null /*
                                                              * ni.getContext ()
                                                              */));
         ...
         * Log.debug("appends("+baseId+","+nextId+"):"+_appends(baseId, nextId)); return
         * _appends(baseId, nextId); }
         */

        private boolean appends(String baseId, String nextId) {
            int sep = baseId.lastIndexOf('/');
            if (sep == -1 || nextId.length() < sep || sep != nextId.lastIndexOf('/') ||
                !nextId.startsWith(baseId.substring(0, sep))) return false;
            if (Integer.parseInt(baseId.substring(sep + 1)) + 1 != Integer.parseInt(nextId.substring(sep + 1)))
                return false;
            return true;
        }
    }
