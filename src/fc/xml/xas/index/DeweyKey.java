...
/**
 * A representation of a Dewey key. A Dewey key is a sequence of non-negative integers, each integer
 * denoting the index of the child to pick next when going deeper in an XML document. Dewey keys are
 * used in XAS because they permit efficient structural comparison. Dewey keys have a one-to-one
 * correspondence with <code>int</code> arrays containing non-negative values. The root key
 * corresponds to an empty array. The class has methods for converting in either direction between
...
public final class DeweyKey {
    ...
    /**
     * Get a Dewey key corresponding to the first child of the root of a tree.
     */
    ...
    public static DeweyKey topLevel(int value) {
        if (value < 0) { throw new IllegalArgumentException("DeweyKey component " + value +
                                                            " negative"); }
     ...
     * @param path
     *            the sequence of indices to append
     */
     ...
     * Create a Dewey key by appending to an existing key.
     * @param path
     *            an array containing the sequence of indices to append
     * @param offset
     *            the offset of the first index in <code>path</code>
     * @param length
     *            the number of indices to append
     */
     ...
     * @param path
     *            the sequence of indices of the created key
     */
     ...
     * Create a Dewey key from an integer array.
     * @param path
     *            an array containing the sequence of indices of the created key
     * @param offset
     *            the offset of the first index in <code>path</code>
     * @param length
    ...
    public static DeweyKey construct(int[] path, int offset, int length) {
        if (path == null || length == 0) { return ROOT; }
        if (offset < 0 || length < 0 || offset + length > path.length) { throw new IllegalArgumentException(
                                                                                                            "offset=" +
                                                                                                                    offset +
                                                                                                                    ",length=" +
                                                                                                                    length); }
    ...
    public DeweyKey up() {
        if (isRoot()) { throw new IllegalStateException("Cannot go up from root DeweyKey"); }
        return parent;
    }


    public DeweyKey next() {
        if (isRoot()) { throw new IllegalStateException("Root DeweyKey does not have a next key"); }
    ...
    public DeweyKey prev() {
        if (isRoot()) {
            throw new IllegalStateException("Root DeweyKey does not have a previous key");
        } else if (value == 0) {
    ...
    public DeweyKey next(DeweyKey ancestor) {
        ...
        if (Util.equals(parent, ancestor)) {
            ...
            throw new IllegalArgumentException("Key " + ancestor + " not an ancestor of " + this);
        } else {
    ...
    public DeweyKey replaceAncestor(DeweyKey old, DeweyKey repl) {
        ...
        if (Util.equals(parent, old)) {
            ...
            throw new IllegalArgumentException("Key " + old + " not an ancestor of " + this);
        } else {
    ...
    public boolean isDescendant(DeweyKey key) {
        Verifier.checkNotNull(key);
        return key.isRoot() || isChild(key) || (!parent.isRoot() && parent.isDescendant(key));
    }
    ...
    public boolean precedeSibling(DeweyKey key) {
        Verifier.checkNotNull(key);
        return !key.isRoot() && Util.equals(parent, key.parent) && value < key.value;
    }
