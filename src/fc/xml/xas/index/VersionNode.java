...
public class VersionNode {
    ...
    FragmentPointer update(FragmentPointer pointer) {
        ...
        switch (kind) {
            ...
            case MOVE:
                ...
                } else if (pointer.behind(sourcePointer) && !pointer.behind(targetPointer)) {
                    result = pointer.translate(-1);
                } else if (pointer.behind(targetPointer) && !pointer.behind(sourcePointer)) {
                    result = pointer.translate(1);
                }
                break;
        }
    ...
    public Index.Entry update(Index.Entry entry) {
        ...
        switch (kind) {
            case INSERT:
                ...
                } else if (between(offset, sourceEntry.getOffset(), sourceEntry.getLength(), length)) {
            ...
            case DELETE:
                ...
                break;
            case MOVE:
                if (targetEntry.getOffset() <= offset && offset < sourceEntry.getOffset()) {
                    newOffset += sourceEntry.getLength();
                } else if (sourceEntry.getEnd() <= offset && offset < targetEntry.getOffset()) {
                    newOffset -= sourceEntry.getLength();
                } else if (offset == sourceEntry.getOffset()) {
                    newOffset = targetEntry.getOffset();
                    newContext = targetEntry.getContext();
                } else if (between(offset, sourceEntry.getOffset(), sourceEntry.getLength(), length) &&
                           !between(offset, targetEntry.getOffset(), targetEntry.getLength(),
                                    length)) {
                    newLength -= sourceEntry.getLength();
                } else if (between(offset, targetEntry.getOffset(), targetEntry.getLength(), length) &&
                           !between(offset, sourceEntry.getOffset(), sourceEntry.getLength(),
                                    length)) {
                    newLength += sourceEntry.getLength();
                } else if (between(sourceEntry.getOffset(), offset, length, sourceEntry.getLength())) {
    ...
    public VersionNode moveAfter(DeweyKey source, FragmentPointer sourcePointer, DeweyKey target,
        ...
        return make(Kind.MOVE, source, sourcePointer, target, targetPointer, true);
    }


    public VersionNode moveTo(DeweyKey source, FragmentPointer sourcePointer, DeweyKey target,
                              FragmentPointer targetPointer) {
        Verifier.checkNotNull(source);
        Verifier.checkNotNull(sourcePointer);
        Verifier.checkNotNull(target);
        Verifier.checkNotNull(targetPointer);
        return make(Kind.MOVE, source, sourcePointer, target, targetPointer, false);
    }
