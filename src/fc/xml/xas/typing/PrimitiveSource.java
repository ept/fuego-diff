            ...
            if (Item.isStartTag(item)) {
                ...
                if (an != null) {
                    ...
                    if (value instanceof String) {
                        ...
                        UnparsedPrimitive up = new UnparsedPrimitive(type, bytes, encoding);
                        ...
                        lookingType = (Qname) ((ParsedPrimitive) value).getValue();
                    } else if (value instanceof Qname) {
                        lookingType = (Qname) value;
                    } else {
                        throw new IllegalStateException("Value of attribute " + an +
                                                        " not convertible to Qname");
                    }
                ...
                if (!UnparsedPrimitive.isUnparsedPrimitive(item)) {
                    ...
                    if (Item.isEndTag(item)) {
                        cachedItem = item;
                        UnparsedPrimitive up = new UnparsedPrimitive(type, bout.toByteArray(),
                                                                     encoding);
                        item = up.convert(lookingType, lookingTag);
                    } else if (bout.size() > 0) {
                        cachedItem = item;
                        item = new Text(new String(bout.toByteArray(), encoding));
                    }
                } else {
