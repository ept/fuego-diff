 ...
 * upon construction is identical to some underlying id-addressable reftree. The tree may then be
 * modified using the {@link MutableRefTree} interface. The underlying tree is known as the
 * <i>back</i> tree.
 * <p>
 * A reftree expressing the tree using references to the back tree is available through the
 * {@link #getChangeTree getChangeTree} method at each point during the change cycle.
 * <p>
 * For instance, consider a change tree with the back tree
 * 
 * <pre>
 ...
 * <code>insert("1","5","&lt;file name='newfile' &gt;")</code>. Calling <code>getChangeTree</code>
 * after the insert would yield the reftree
 * 
 * <pre>
 * &lt;ref:node id=&quot;1&quot;&gt;
 *   &lt;ref:tree id=&quot;2&quot;  /&gt;
 ...
 * </pre>
 * <p>
 * This class is in some senses the dual of
 * {@link RefTrees#apply(RefTree, MutableRefTree, Set, RefTrees.IdMap)}: the former applies a
 * reftree as a series of operations on a mutable reftree, the latter expresses the result of a
 * series of operations on a mutable reftree as a reftree.
 * <p>
 * Note that the implementation does not guarantee that a minimal (in terms of nodes) reftree is
 * returned by <code>getChangeTree</code>; changes that cancel out may yield a reftree with more
 * than a single tree reference to the root of the underlying tree.
 * <p>
 * <b>Note:</b> Be careful to always update node content via the {@link #update update} method.
 * Accessing the <code>RefTreeNode</code>s directly may not be intercepted by the class, and may
 * change the underlying tree.
 * <p>
 */
...
public class ChangeBuffer extends DerefRefTree implements MutableRefTree, BatchMutable {

    protected Set<Key> delRoots = new HashSet<Key>(); // These contain back
    // keys!
    protected Set<Key> delNodes = new HashSet<Key>(); // These contain back
    // keys!
    ...
    public ChangeBuffer(IdAddressableRefTree backingTree, KeyMap kmap) {
        this(new MutableRefTreeImpl(kmap.getFrontKey(backingTree.getRoot().getId()),
                                    MutableRefTreeImpl.ID_AS_STRINGS_BY_LENGTH_ALPHA), backingTree,
             kmap);
    }


    // NOTE: Will use changeTree as keymap, if it is a keymap
    public ChangeBuffer(MutableRefTree changeTree, IdAddressableRefTree backingTree) {
        this(changeTree, backingTree, changeTree instanceof KeyMap ? (KeyMap) changeTree
                : KeyMap.IDENTITY_MAP);
    }


    public ChangeBuffer(MutableRefTree changeTree, IdAddressableRefTree backingTree, KeyMap map) {
     ...
     * @param changeTree
     *            tree to buffer changes in
     * @param backingTree
     *            backing tree
     * @param map
     *            keymap between change and backing trees
     * @param changeTreeRoot
     *            key to use for root in changeTree. If set to {@link MutableRefTree#AUTO_KEY} (the
     *            default), the mapped root key of the backing tree will be used.
     */
    public ChangeBuffer(MutableRefTree changeTree, IdAddressableRefTree backingTree, KeyMap map,
                        Key changeTreeRoot) {
    ...
    /**
     * Get the tree as a reftree. The reftree references nodes in the underlying tree.
    ...
    /**
     * Reset tree. Resets all changes made to the tree, causing the tree to be identical to the back
     * tree.
     */
    ...
    public void reset() {
        ...
        try {
            ...
            if (fid != null && changeTree.contains(fid)) changeTree.delete(fid);
            fid = km.getFrontKey(backingTree.getRoot().getId());
            changeTree.insert(null, changeTreeRoot != MutableRefTree.AUTO_KEY ? changeTreeRoot
                    : fid, TreeReference.create(bid));
    ...
    /** @inheritDoc */
    public final Key insert(Key parentId, Key newId, Object content) throws NodeNotFoundException {
        return insert(parentId, DEFAULT_POSITION, newId, content);
    }


    /** @inheritDoc */
    public Key insert(Key parentId, long pos, Key newId, Object c) throws NodeNotFoundException {
        if (contains(newId)) throw new IllegalArgumentException("Node already in tree " + newId);
        ...
        if (c instanceof Reference) removeFromDeletia((Reference) c);
    ...
    public boolean update(Key nodeId, Object c) throws NodeNotFoundException {
        expand(nodeId, true); // We need child expand since we update the
        // content
        ...
        if (changeTree.getNode(nodeId).isNodeRef()) {
            ...
            if (updateLoadsContent || !same) changeTree.update(nodeId, same ? cb : c);
        } else same = changeTree.update(nodeId, c);
        if (c instanceof Reference) removeFromDeletia((Reference) c);
    ...
    public void apply(RefTree t) throws NodeNotFoundException {
        ...
        Set[] usedRefs = RefTrees.normalize(backingTree, new RefTree[] { t, backRef },
        ...
        Set newDelNodes = new HashSet();
        {
            ...
            for (queue.add(t.getRoot()); !queue.isEmpty();) {
                RefTreeNode n = queue.removeLast();
                if (n.isNodeRef()) newDelNodes.remove(n.getReference().getTarget());
                for (Iterator i = n.getChildIterator(); i.hasNext();)
                    queue.add((RefTreeNode) i.next());
            }
        }
    ...
    protected void expandAll(RefTreeNode n, KeyMap km) throws NodeNotFoundException {
        ...
        Key toExpand = n.isReference() ? (n.getReference().getTarget()) : km.getBackKey(n.getId());
        if (toExpand != null) {
    ...
    protected void addToDeletia(RefTreeNode n) {
        if (n.isReference()) {
            ...
            // TODO: Having an insert flag for nodes could eliminate checking
            // with
            // backTree for update/ins status of node
            Key bk = km.getBackKey(n.getId());
            if (bk != null && backingTree.contains(bk)) delNodes.add(bk);
        }
        for (Iterator i = n.getChildIterator(); i.hasNext();)
    ...
    protected void removeFromDeletia(Reference ref) throws NodeNotFoundException {
        Key target = ref.getTarget();
        if (!backingTree.contains(target)) throw new NodeNotFoundException(target);
        if (!ref.isTreeReference()) {
            if (delNodes.remove(target)) {
                ...
                // More complex case: undelete node-ref and it is an
                // undelete-tree -->
            ...
            if (delRoots.remove(target)) {
                ...
                // Undeleted tree-ref, and the root was marked deleted -> remove
                // any
                // delete-marked descendants of the ref from the sets. We need
                // to check
                // if any deleted node is a descendant of this, in which case it
                // is
                // no longer in the delete set
                // NOTE: here, we can remove nodes from nodeSet w/o adding its
                // children
                // to delRoots, as those nodes will be included by the the
                // treeRef n
                assert !delRoots.contains(target);
    ...
    protected boolean isDeleted(Key bk) {
        assert backkey(bk);
        if (delNodes.contains(bk)) return true;
        try {
            // Check that nothing on the path to the root is deleted
            for (; bk == null || backingTree.contains(bk); bk = backingTree.getParent(bk)) {
                if (bk == null) return false;
                if (delRoots.contains(bk)) return true;
            }
    ...
    // expanded childlists)
    protected Key expand(Key eid, boolean expandChildren) throws NodeNotFoundException {
        // Log.debug("Changetree before expand of "+eid);
        // XmlrDebug.dumpTree(getChangeTree());
        if (eid == null) throw new NodeNotFoundException(eid);
        if (changeTree.contains(eid) && !changeTree.getNode(eid).isTreeRef()) return eid; // Trivial
        // case
        // when
        // eId is
        // already
        ...
        for (Key backId = km.getBackKey(id); id != null;) {
            // BUGFIX-20061009-0: KeyMaps may return keys not in backing tree,
            // so
            // we need to do a contains check
            // Log.debug("Back key of "+id+" is "+backId+", eid="+eid);
            if (id == eid && !backingTree.contains(backId)) throw new NodeNotFoundException(id);
            assert id != eid || backId != null // Assert for the above if
            : "Trying to expand key " + id + ", which is not in tree, and has no backKey";

            if (changeTree.contains(id)) {
                assert id == null || changeTree.getNode(id).isTreeRef();
                Key pId = changeTree.getParent(id);
                assert pId == null || changeTree.getNode(pId) != null : "ChangeTree returned non-existing parent " +
                                                                        pId + " of " + id;
                assert pId == null || !changeTree.getNode(pId).isTreeRef() : "treeref as parent of treeref in expandTree";
                // Node expandRoot = null;
                // Log.debug("Expansion phase, expand stack is",expandStack);
                /*
                 * BUGFIX-20070215-1: Do not expand children when node already expanded and
                 * expandChildren = false. If eid was already in changeTree, we'd get here
                 * immediately. Then the for loop would expand the children of eid=id, regardless of
                 * expandChildren status; "&& expandChildren" fixes this. In other cases, it's still
                 * safe since expandChildren is set to true on the first step upwards.
                 */
                for (; id != null && expandChildren; id = expandStack.isEmpty() ? null
                        : expandStack.pop()) {
                    // Log.debug("Expanding subtree "+id+" whose node now is "+
                    // changeTree.getNode(id));
                    assert changeTree.getNode(id).isTreeRef(); // Only expand
                    // treerefs
                    changeTree.update(id, NodeReference.create(km.getBackKey(id)));
                    /*
                     * changeTree.delete(id); changeTree.insert( pId, id, new
                     * RefTreeNodeImpl.NodeReference( km.getBackKey( id ) ) );
                     */
                    assert changeTree.getNode(id).isNodeRef() : "Non-std semantic for node-ref was busted";
            ...
            // [p0-p1-p2-...-p(n-1)]-pn-m
            expandStack.push(id);
            else expandChildren = true; // This leaves out id given at call from
            // stack if
            ...
            assert backId != null : "Found null backId->this front-key is now deleted?!: " + eid +
                                    "(now at front-key " + id +
                                    "). This may happen if the contains() method of your backing " +
    ...
    private void insertRefSubtree(MutableRefTree ft, IdAddressableRefTree bt, Key froot)
        ...
        for (Iterator i = bt.childIterator(km.getBackKey(froot)); i.hasNext();) {
            ...
            assert ft.getNode(km.getFrontKey(bckey)).isTreeRef() : "TreeRef insert broken";
        }
    }
