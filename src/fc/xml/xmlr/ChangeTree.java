 ...
 * modified using the {@link MutableRefTree} interface.
 * <p>
 * <b>NOTE:</b> Newer implementations should use {@link fc.xml.xmlr.ChangeBuffer}, which provides
 * the same functionality, but has a more solid and modular implementation. This class will soon be
 * deprecated.
 * <p>
 * A reftree expressing the tree using references to the underlying tree is available through the
 * {@link #getChangeTree getChangeTree} method at each point during the change cycle.
 * <p>
 * For instance, consider a change tree with the underlying tree
 * 
 * <pre>
...
public class ChangeTree extends AbstractMutableRefTree {
    ...
    /**
     * Create a new change tree. The tree is initially identical to the underlying tree.
     * @param backingTree
     *            underlying tree
     */
    ...
    /**
     * Create a new change tree. The tree is initially identical to the underlying tree. The changes
     * to this tree are mirrored to <code>changeTarget</code>.
     * @param backingTree
     *            underlying tree
     * @param changeTarget
     *            tree to which any changes are mirrored. May be same as <code>backingTree</code>.
     */
    ...
    /**
     * Reset tree. Resets all changes made to the tree, causing the tree to be identical to its
     * underlying tree.
     */
    ...
    protected RefTreeNode getNode(Key id, boolean transparent) {
        ...
        if (n != null) n = new ProxyNode(n, transparent);
    ...
    protected boolean isDeleted(RefTreeNode n) {
        ...
        if (delRoots.contains(n.getId())) return true;
        // __deletdepth++;
        // if( __deletdepth > 100 )
        // Log.log("Infinite delete recursion :(",Log.ASSERTFAILED);
        if (isDeleted(n.getParent())) {
            // delRoots.add(n.getId()); // Cache whole path to node as deleted
            // -> faster look-up
            // Actually, pretty useless: most nodes are not deleted -> must scan
            // to
            // root :( anyway
            // __deletdepth--;
            return true;
    ...
    public void delete(Key id) throws NodeNotFoundException {
        ...
        if (p == null) throw new IllegalArgumentException("Cannot delete root");
        ...
        changeCount++;
        // Propagate change
        if (changeTarget != null) changeTarget.delete(id);
    }
    ...
    public Key insert(Key parentId, long pos, Key newId, Object content)
        ...
        delRoots.remove(newNode.getId()); // BUGFIX 010906: Remove the node from
        // the deletia
        ...
        // Propagate change
        if (changeTarget != null) changeTarget.insert(parentId, pos, newId, content);
        return newId;
    }
    ...
    public Key move(Key nodeId, Key parentId, long pos) throws NodeNotFoundException {
        ...
        if (n == null) {
            ...
            Key origParentId = ki.getFrontKey(backingTree.getNode(ki.getBackKey(nodeId)).getParent().getId());
        ...
        changeCount++;
        // Propagate change
        if (changeTarget != null) changeTarget.move(nodeId, parentId, pos);
        return nodeId;
    }
    ...
    public boolean update(Key nodeId, Object content) throws NodeNotFoundException {
        ...
        if (content.equals(oldContent)) {
            // taint(nodeId,null,true); // This is actually somewhat
            // questionable!!!!
            // It means that if you touch a node with upd, you expand it
            // OTOH, this makes RefTrees.apply() work due to its access pattern
            return false;
        }
        // Log.log("---------Old content is ",Log.INFO,
        // fc.syxaw.proto.Util.toString( oldContent ));
        // Log.log("---------New content is ",Log.INFO,
        // fc.syxaw.proto.Util.toString( content ));
        taint(nodeId, content, true);
        changeCount++;
        // Propagate change
        if (changeTarget != null) changeTarget.update(nodeId, content);
        return true;
    }
    ...
    protected void verifyMove(Key id, Key newParent) {
        if (Util.equals(id, newParent)) {
            // Log.log("Moving node to be child of its own subtree",
            // Log.ASSERTFAILED);
    ...
    protected RefTreeNode taint(Key id, Object content, boolean expandChildren)
        ...
        /*
         * java.io.PrintWriter pw = new java.io.PrintWriter(System.out); dump(pw,refRoot);
         * pw.flush();
         */
        return n;
    }
    ...
    private/* static */class Node implements RefTreeNode {
        ...
        private void expandChildren(IdAddressableRefTree target, Key id)
            ...
            for (Iterator<Key> i = target.childIterator(id); i.hasNext();) {
                ...
                RefTreeNode child = new Node(fcid, this, null, true, false);
    ...
    private class ProxyNode implements RefTreeNode {
        ...
        public RefTreeNode getParent() {
            ...
            RefTreeNode p = findNode(n instanceof Node ? n.getId() : ki.getFrontKey(n.getId()));
            if (p != null) p = p.getParent();
            else p = n.getParent();
            return p != null ? new ProxyNode(p, transparent) : null;
        }
        ...
        public Object getContent() {
            ...
            /*
             * OLD if( n instanceof Node && content == null && transparent ) return
             * backingTree.getNode(n.getId()).getContent();
             */
            if (n instanceof Node && !((Node) n).contentExpanded() && transparent) {
        ...
        public Iterator getChildIterator() {
            ...
            final Iterator niter = n instanceof Node && transparent &&
                                   !((Node) n).childrenExpanded() ? backingTree.getNode(
                                                                                        ki.getBackKey(n.getId())).getChildIterator()
                    : n.getChildIterator();
            return new Iterator() {
        ...
        public boolean isTreeRef() {
            if (/* n instanceof Node && !((Node) n).childrenExpanded() && */transparent) {
                RefTreeNode n2 = backingTree.getNode(n instanceof Node ? ki.getBackKey(n.getId())
                        : n.getId());
        ...
        public Reference getReference() {
            ...
            return !isReference() ? null : (isTreeRef() ? new TreeReference(n.getId())
    ...
    private abstract static class MutableMappingIndex { // FIXME: Obviously
        ...
        public abstract void move(Key fk, RefTreeNode n); // if fk not
