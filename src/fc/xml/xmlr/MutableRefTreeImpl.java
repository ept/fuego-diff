...
public class MutableRefTreeImpl extends AbstractMutableRefTree {
    ...
    /**
     * Comparator ordering strings by ascending length, then by the default Java String order.
     */
    public static final Comparator ID_AS_STRINGS_BY_LENGTH_ALPHA = new Comparator() {

        public int compare(Object o1, Object o2) {
            Key id1 = ((RefTreeNode) o1).getId();
            Key id2 = ((RefTreeNode) o2).getId();
            return id1 == null ? (id1 == id2 ? 0 : -1)
                    : CompareUtil.STRINGS_BY_LENGTH_ALPHA.compare(id1.toString(), id2.toString());
        }
    };
    ...
    /**
     * Create tree with a given root key. Child ordering is {@link #ALWAYS_LAST} .
     */
    ...
    public Key insert(Key parentId, long pos, Key newId, Object content)
        ...
        if (p == null && parentId != null) throw new NodeNotFoundException(parentId);
    ...
    public Key move(Key nodeId, Key parentId, long pos) throws NodeNotFoundException {
        ...
        Node p = (Node) nodeByKey.get(parentId);
        if (p == null || n == null) throw new NodeNotFoundException(n != null ? parentId : nodeId);
        ((Node) n.getParent()).removeChild(n);
        if (pos == DEFAULT_POSITION) pos = getPosFor(p, n);
        p.addChild((int) pos, n); // Possible precision loss...
        return n.getId();
    }
    ...
    protected int getPosFor(Node parent, Node n) {
        int pos = 0;
        if (childPosCmp == ALWAYS_LAST) return parent.childCount();
        for (Iterator i = parent.getChildIterator(); i.hasNext();) {
            if (childPosCmp.compare(n, i.next()) > 0) pos++;
            else return pos;
        }
        return pos;
    }
    ...
    private static class Node extends RefTreeNodeImpl {
        ...
        /*
         * @Override protected List makeList() { return new LinkedList(); }
         */
