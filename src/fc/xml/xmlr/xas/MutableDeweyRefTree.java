...
public class MutableDeweyRefTree extends AbstractMutableRefTree implements KeyMap,
  ...
  private Map<DeweyKey,DeweyRefNode> nodeRefTab = 
    new HashMap<DeweyKey,DeweyRefNode>();
  private Map<DeweyKey,DeweyRefNode> treeRefTab = 
    new HashMap<DeweyKey,DeweyRefNode>();
  ...
  public void apply(RefTree t) throws NodeNotFoundException {
    ...
    try {
      setRoot(apply(t.getRoot(),new DeweyRefNode.PrefixNode(
          forceAutoKey ? DeweyKey.ROOT_KEY :
  ...
  public Key insert(Key parentId, long pos, Key newId, Object content)
    ...
    DeweyRefNode n = null;
    if( p == null ) {
      DeweyKey rootKey =  ( !forceAutoKey && newId != AUTO_KEY ) ?
        (DeweyKey) newId : DeweyKey.ROOT_KEY;
    ...
    if( !forceAutoKey && newId != AUTO_KEY 
  ...
  protected DeweyRefNode find(fc.xml.xas.index.DeweyKey k, int left ) {
    if(left < 1 ) {
      if( Util.equals( rootKey.getXasDeweyKey(),k ) )
        return root;
      else
        return null;
    } else {
  ...
  private Key _getFrontKey(Key bk) {
    ...
    DeweyKey mapped = mappedNode == null ? null : 
      (DeweyKey) nodeRefTab.get(backKey).getId();
    if( mapped != null ) {
