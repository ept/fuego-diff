...
public interface UniformXasCodec extends XasCodec {
    ...
    /**
     * Size of content in items, if known. The method should return the uniform size in items of
     * content (if known). Return <code>-1</code> if the size is unknown, or if it varies. Some code
     * using a TreeModel requires fixed size content (see the Javadoc of the class in question).
    ...
    public static class ItemCodec implements UniformXasCodec {
        ...
        public Object decode(PeekableItemSource is, KeyIdentificationModel kim) throws IOException {
            Item i = is.next();
            if (i.getType() == TypedItem.TYPED) return ((TypedItem) i).getValue();
            return i;
        }
