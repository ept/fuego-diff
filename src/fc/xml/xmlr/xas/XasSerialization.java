...
public class XasSerialization {
    ...
    // BUGFIX-20061016-2:Use ItemList i.s.t.o.f. ItemSource, as it is
    // re-consumable
    /**
     * Default UTF-8 document header. The value is <code>[StartDocument.instance()]</code>
     */
    public static final ItemList DOC_HEADER_UTF8 = (new ItemList(StartDocument.instance()));

    /**
     * Default UTF-8 document trailer. <code>[EndDocument.instance()]</code>
     */
    public static final ItemList DOC_TRAILER_UTF8 = (new ItemList(EndDocument.instance()));
     ...
     */
    public static RefTree readTree(ItemSource s, XasCodec xc) throws IOException {
     ...
     *            item source to read from
     * @param tm
     *            tree model for building tree
     * @return read tree
     * @throws IOException
     *             if an I/O error occurs.
     */
    public static RefTree readTree(ItemSource s, TreeModel tm) throws IOException {
        List<Item> header = new LinkedList<Item>();
        List<Item> trailer = new LinkedList<Item>();
        PeekableItemSource ps = s instanceof PeekableItemSource ? (PeekableItemSource) s
                : new PeekableItemSource(s);
        // Read header
        // BUGFIX-20070625-2: Properly read trees with pre-decoded RefItems
        for (Item i = ps.peek(); i != null &&
                                 (i.getType() != Item.START_TAG && !RefItem.isRefItem(i)); i = ps.peek())
            header.add(ps.next());
        RefTree tree = new RefTreeImpl(readTree(null, ps, tm, 0,
    ...
    /**
     * Write reftree. Uses the tree model <code>TreeModels.xmlr1Model().swapCodec(xc)</code>, and
     * the default header and trailer.
     * @param tree
     *            tree to write
     * @param out
     *            item output
     ...
     * @throws IOException
     */
    public static void writeTree(RefTree tree, ItemTarget out, XasCodec xc) throws IOException {
        writeTree(tree, out, TreeModels.xmlr1Model().swapCodec(xc),
                  XasUtil.itemSource(DOC_HEADER_UTF8), XasUtil.itemSource(DOC_TRAILER_UTF8));
    }


    /**
     * Write reftree. Uses the default header and trailer.
     * @param tree
     *            tree to write
     * @param out
     *            item output
     ...
     * @throws IOException
     */
    public static void writeTree(RefTree tree, ItemTarget out, TreeModel tm) throws IOException {
        writeTree(tree, out, tm.getCodec(), tm, XasUtil.itemSource(DOC_HEADER_UTF8),
     ...
     * @throws IOException
     */
    public static void writeTree(RefTree tree, ItemTarget out, TreeModel tm, ItemSource header,
                                 ItemSource trailer) throws IOException {
     ...
     */
    public static void writeTree(RefTree tree, ItemTarget out, XasCodec c,
                                 KeyIdentificationModel kim, ItemSource header, ItemSource trailer)
            throws IOException {
    ...
    protected static RefTreeNode readTree(RefTreeNode parent, PeekableItemSource s, TreeModel tm,
                                          int childPos, boolean customRefCodec) throws IOException {
        Item rawfirst = s.peek(), first = rawfirst;
        // Log.debug("readTree @ "+rawfirst);
        if (!customRefCodec) first = RefItem.decode(first);
        Reference ref = null;
        ...
        if (!customRefCodec && RefItem.isRefItem(first)) {
            ...
            s.next();
            content = ri.isTreeRef() ? new TreeReference(tm.makeKey(ri.getTarget()))
                    : new NodeReference(tm.makeKey(ri.getTarget()));
            // BUGFIX-20061107-20: we did not consume treeref close tag, as
            // described
            // below
            // Check if we decoded a treeref item (as opposed to treeref items
            // in the underlying itemSource), in which case we need to eat the
            // ET()
            if (ri.isTreeRef() && first != rawfirst) {
                if (RefItem.whatItem(s.peek()) != RefItem.ITEM_TREEREF_CLOSE) { throw new IOException(
                                                                                                      "Expected " +
                                                                                                              new EndTag(
                                                                                                                         RefItem.REF_TAG_TREE) +
                                                                                                              "got " +
                                                                                                              s.next()); }
                s.next();
            }
            ...
            assert !(tm.getCodec() instanceof UniformXasCodec) || s.getLeft() == 0 : "Tree decoder violated size agreement of " +
                                                                                     ((UniformXasCodec) tm.getCodec()).size() +
                                                                                     " items.";
        }
        ...
        if (first.getType() == Item.START_TAG || n.isNodeRef()) {
            ...
            for (; last != null && last.getType() != Item.END_TAG &&
                   !(last instanceof RefNodeItem && ((RefNodeItem) last).isEndTag()); last = s.peek()) {
            ...
            if (n.isNodeRef()) {
                ...
                if (nodeRefEnd.getType() != RefNodeItem.NODE_REFERENCE ||
                    ...
                    throw new IOException("Expected end of node reference " + ref + ", got " +
                                          nodeRefEnd);
            } else if (last != null && last.getType() == Item.END_TAG) s.next(); // Eat ET
        }
        return n;
    }


    protected static void writeTree(RefTreeNode node, AutoET out, XasCodec codec,
                                    boolean customRefTags) throws IOException {
        ...
        if (emitRef) {
            ...
            out.append(ri);
        } else codec.encode(out, node, out.context());
        assert !(codec instanceof UniformXasCodec) || out.getLeft() == 0 : "Tree encoder violated size agreement of " +
                                                                           ((UniformXasCodec) codec).size() +
                                                                           " items.";
    ...
    private static class AutoET implements ItemTarget {
        ...
        public void autoET() throws IOException {
            ...
            for (Item i = lastST.pop(); i != MARKER; i = lastST.pop()) {
                if (Item.isStartTag(i)) dst.append(new EndTag(((StartTag) i).getName()));
            }
            Item ci = lastST.size() > 0 ? lastST.peek() : null;
            // Scan for previous context by going towards the stack top
            // A bit ugly, as it relies on the list-ish Java stack
            // implementation
