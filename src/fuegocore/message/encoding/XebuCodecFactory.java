...
/**
 * A factory for Xebu serializers and parsers. The serializers and parsers produced by this factory
 * use the Xebu format, the precise features of which are determined by the name of the format.
 * There are currently six different factories, one for each feature turned on or off. The features
 * are item caching, content caching, and element caching (content caching implies item caching, so
 * there are not eight factories).
 * <p>
 * The names of the formats and associated features are:
 * <ul>
 * <li><code>application/x-ebu+none</code>: no caching</li>
 * <li><code>application/x-ebu+item</code>: item caching</li>
 * <li><code>application/x-ebu+data</code>: item and content caching</li>
 * <li><code>application/x-ebu+elem</code>: element caching</li>
 * <li><code>application/x-ebu+elit</code>: element and item caching</li>
 * <li><code>application/x-ebu+elid</code>: element, item, and content caching</li>
 * </ul>
 * The features are meaningful only for the serializers; parsers determine the needed caching from
...
public class XebuCodecFactory implements TokenCacheCodecFactory {
    ...
    private static final String[] types = { "application/x-ebu+none", "application/x-ebu+item",
    ...
    /**
     * Return a new Xebu parser. The parser is able to parse any Xebu document independently of
     * whether the creating factory has caching features set. The argument token is used to
     * associate previously-used caches with new parsers.
     * @param token
     *            a token identifying the caches to use
     */
    public TypedXmlParser getNewDecoder(Object token) {
        ...
        if (sequenceCache == null && cacheSequence(type)) {
            inSeqCaches.put(token,
                            result.getProperty(XebuConstants.PROPERTY_INITIAL_SEQUENCE_CACHE));
        }
        if (codecFactories != null && codecFactories.size() > 0) {
            ...
            for (Enumeration e = codecFactories.elements(); e.hasMoreElements();) {
    ...
    /**
     * Return a new Xebu serializer. The returned serializer will use caches according to the
     * features of the creating factory and write the corresponding Xebu format. The argument token
     * is used to associate previously-used caches with new serializers.
     * @param token
     *            a token identifying the caches to use
     */
    public TypedXmlSerializer getNewEncoder(Object token) {
        ...
        if (sequenceCache == null && cacheSequence(type)) {
            outSeqCaches.put(token,
                             result.getProperty(XebuConstants.PROPERTY_INITIAL_SEQUENCE_CACHE));
        }
        result.setFeature(XebuConstants.FEATURE_ITEM_CACHING, cacheItem(type));
        result.setFeature(XebuConstants.FEATURE_CONTENT_CACHING, cacheContent(type));
        result.setFeature(XebuConstants.FEATURE_SEQUENCE_CACHING, cacheSequence(type));
        if (codecFactories != null && codecFactories.size() > 0) {
     ...
     * @param token
     *            a token identifying the cache to set
     * @param cache
     *            the initial cache to use
     * @return <code>true</code> if the cache of <code>token</code> was set to <code>cache</code>,
