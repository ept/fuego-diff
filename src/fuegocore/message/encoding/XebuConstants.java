...
/**
 * Various constants used for Xebu.  This class collects the
 * constant values needed by the Xebu serializer and parser.  This
 * includes identifier tokens for event types, cache identifiers, and
 * common strings.
 */
public class XebuConstants {
    ...
    /**
     * A token indicating that the proper type of the event is coded
     * into the flags.  This provides a bit more extendibility when
     * possibly adding tokens in the future.
     */
    public static final int SPECIAL = 0x0F;

    /**
     * The bitmask to use when extracting the type of a {@link
     * #SPECIAL} token from the flags.
     */
    ...
    /**
     * The size of one item cache.  This size is tightly coupled with
     * the Xebu specification and must not be increased without
     * serious consideration of the consequences.
     */
    public static final int CACHE_SIZE = 0x100;

    /**
     * The size of the sequence cache.  This size is tightly coupled
     * with the Xebu specification and must not be increased without
     * serious consideration of the consequences.
     */
    ...
    /**
     * The flag for fetching a namespace-name-value triplet or content
     * from its item cache.
     */
    public static final int VALUE_FETCH = 0xC0;

    /**
     * The flag to indicate a {@link #DATA_START} event may be
     * coalesced with a previous {@link #DATA_START} event.
     */
    public static final int COALESCE_FLAG = 0x20;

    /**
     * The flag to indicate that a {@link #TYPED_DATA} event contains
     * more than one encodings of typed content.
     */
     ...
     */
    public static final String FEATURE_ITEM_CACHING =
	"http://www.hiit.fi/fuego/fc/xebu/item-cache";

    /**
     * The flag to use with the {@link #DOCUMENT} token to indicate
     * use of item caching in the document.
     */
     ...
     */
    public static final String FEATURE_CONTENT_CACHING =
	"http://www.hiit.fi/fuego/fc/xebu/content-cache";

    /**
     * The flag to use with the {@link #DOCUMENT} token to indicate
     * use of content caching in the document.
     */
     ...
     */
    public static final String FEATURE_SEQUENCE_CACHING =
	"http://www.hiit.fi/fuego/fc/xebu/elem-cache";

    /**
     * The flag to use with the {@link #DOCUMENT} token to indicate
     * use of sequence caching in the document.
     */
     ...
     */
    public static final String PROPERTY_INITIAL_CACHES =
	"http://www.hiit.fi/fuego/fc/xebu/init-cache";

    /**
     * The property to use for setting the initial sequence cache.
     */
    public static final String PROPERTY_INITIAL_SEQUENCE_CACHE =
	"http://www.hiit.fi/fuego/fc/xebu/init-elem-cache";

    /**
     * The property to use for setting at what depth does the element
     * cacher start caching elements.
     */
    public static final String PROPERTY_SEQUENCE_CACHE_DEPTH =
	"http://www.hiit.fi/fuego/fc/xebu/elem-cache-depth";

    /**
     * The property to use for setting EOA and DOA machines for
     * serializers and parsers.
     */
    public static final String PROPERTY_COA_MACHINE =
	"http://www.hiit.fi/fuego/fc/xebu/coa-machine";

    /**
     * The special flag value for a processing instructions.
     * Processing instructions are cached into the same cache as
     * content, and their caching also depends on {@link
     * #FEATURE_CONTENT_CACHING}.
     */
    public static final int FLAG_PI = 0x00;

    /**
     * The special flag value for a comment.  Comments are never
     * cached, since there would probably be no benefit in that.
     */
    public static final int FLAG_COMMENT = 0x10;

    /**
     * The special flag value for an entity reference.  Entity
     * references will be cached into the same cache as content is,
     * except that their caching depends on {@link
     * #FEATURE_ITEM_CACHING}, since caching entity references is more
     * useful in general than caching content.
     */
