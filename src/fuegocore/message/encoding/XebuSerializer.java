...
/**
 * A serializer for Xebu documents. This class implements the output of Xebu documents as described
 * in the Xebu specification. The parsing interface is {@link TypedXmlSerializer}, an extension to
 * XmlPull's <code>XmlSerializer</code>. The parser implements all caching features of the Xebu
 * format, each of which can be turned on or off with the use of {@link #setFeature}.
 */
public class XebuSerializer implements TypedXmlSerializer, ContentEncoder {
    ...
    private boolean isAcceptable(char[] c, int offset, int length) {
        for (int i = 0; i < length; i++) {
            if (!isAcceptable(c[offset + i])) { return false; }
        }
        return true;
    }
    ...
    private int insertIntoCache(int cacheIndex, Object key) {
        ...
        if (cacheSequence && depth > elemDepth) {
            ...
            tentativeValues = Util.ensureCapacity(tentativeValues, 3 * (tsp + 1));
    ...
    private void putNamespace(String namespace, int token) throws IOException {
        ...
        if (value >= 0) {
            ...
            put(token);
            if (cacheItem) {
                value = insertIntoCache(XebuConstants.NAMESPACE_INDEX, namespace);
                put(value);
            }
    ...
    private void putName(String namespace, String name, int token) throws IOException {
    ...
    private void putValue(String namespace, String name, String value, int token)
            throws IOException {
    ...
    private void putTypedData() throws IOException {
        if (typedData.length() > 0) {
            ...
            /*
             * int value = caches[XebuConstants.CONTENT_INDEX].fetch(data); if (value >= 0) {
             * put(token | XebuConstants.VALUE_FETCH); put(value); } else {
             */
            put(token);
            /*
             * if (cacheContent && isMultiData) { value =
             * insertIntoCache(XebuConstants.CONTENT_INDEX, data); put(value); }
             */
            if (isMultiData) {
                putData(data);
            } else {
        ...
        if (next != null) {
            switch (next.getType()) {
                case Event.START_ELEMENT:
                    putTypedData();
                    putName(next.getNamespace(), next.getName(), XebuConstants.ELEMENT_START);
                    writingAttributes = true;
                    break;
                case Event.ATTRIBUTE:
                    putValue(next.getNamespace(), next.getName(), (String) next.getValue(),
                             XebuConstants.ATTRIBUTE);
                    writingAttributes = true;
                    break;
                ...
                case Event.CONTENT: {
                    if (!writingContent) {
                        ...
                        putContent((String) next.getValue(), XebuConstants.COALESCE_FLAG);
                    }
                    break;
                }
                ...
                case Event.PROCESSING_INSTRUCTION: {
                    putTypedData();
                    int value = caches[XebuConstants.CONTENT_INDEX].fetch(next.getValue());
                    if (value >= 0) {
                        ...
                        if (cacheContent) {
                            value = insertIntoCache(XebuConstants.CONTENT_INDEX, next.getValue());
                            put(value);
                        }
                        putData((String) next.getValue());
                    }
                ...
                case Event.ENTITY_REFERENCE: {
                    putTypedData();
                    int value = caches[XebuConstants.CONTENT_INDEX].fetch(next.getName());
                    if (value >= 0) {
                        ...
                        if (cacheItem) {
                            value = insertIntoCache(XebuConstants.CONTENT_INDEX, next.getName());
                            put(value);
                        }
    ...
    XebuSerializer(OutCache[] caches, SequenceCache sequenceCache, EoaMachine eoa) {
        this.caches = caches;
    ...
    public void setFeature(String name, boolean state) {
        if (XebuConstants.FEATURE_ITEM_CACHING.equals(name)) {
            ...
            checkInProgress();
            if (state && !cacheItem) { throw new IllegalStateException("Property CONTENT_CACHING "
    ...
    public void setProperty(String name, Object value) {
        if (value == null) { throw new IllegalArgumentException("Property must be non-null"); }
        if (XebuConstants.PROPERTY_INITIAL_CACHES.equals(name)) {
            checkInProgress();
            /*
             * if (!cacheItem) { throw new IllegalStateException("Initial caches may not be " +
             * "set with item caching off"); }
             */
            if (!(value instanceof OutCache[])) { throw new IllegalArgumentException(
                                                                                     "Value " +
                                                                                             value +
                                                                                             " is " +
                                                                                             "not a valid initial " +
            ...
            checkInProgress();
            if (!cacheSequence) { throw new IllegalStateException("Initial element cache may "
                                                                  + "not be set with element "
                                                                  + " caching off"); }
            if (!(value instanceof SequenceCache)) { throw new IllegalArgumentException(
                                                                                        "Value " +
                                                                                                value +
                                                                                                " is " +
                                                                                                "not a valid initial " +
            ...
            checkInProgress();
            if (!cacheSequence) { throw new IllegalStateException("Element caching depth may "
                                                                  + "not be set with element "
                                                                  + " caching off"); }
            ...
            checkInProgress();
            if (!(value instanceof EoaMachine)) { throw new IllegalArgumentException(
                                                                                     "Value " +
                                                                                             value +
                                                                                             " is " +
                                                                                             "not a valid COA " +
            ...
            checkInProgress();
            if (!(value instanceof ContentEncoder)) { throw new IllegalArgumentException(
                                                                                         "Value " +
                                                                                                 value +
                                                                                                 " is " +
                                                                                                 "not a valid content " +
    ...
    public String getPrefix(String namespace, boolean generatePrefix) {
        for (int i = 2 * (namespaceCounts[depth] - 1); i >= 0; i -= 2) {
            if (Util.equals(namespace, namespaceStack[i])) { return (String) namespaceStack[i + 1]; }
        }
        return null;
    }
        ...
        if (cacheSequence && depth > elemDepth) {
            ...
            if (value != -1) {
                if (len <= sequenceSize - validIndex) {
                    ...
                    put(XebuConstants.SEQUENCE_FETCH | ((value & 0xF00) >>> 4));
                    put(value & 0xFF);
                    while (tsp > 0) {
                        if (tentativeValues[3 * tsp - 2] >= index) {
                            tsp -= 1;
                            int cacheIndex = tentativeValues[3 * tsp];
                            caches[cacheIndex].tentativeCancel(tentativeKeys[tsp],
                ...
                // XebuConstants.SEQUENCE_CACHE_SIZE
    ...
    public XmlSerializer text(char[] ch, int start, int length) throws IOException {
    ...
    public TypedXmlSerializer typedContent(Object content, String namespace, String name)
        ...
        if (encoder != null && encoder.encode(content, namespace, name, this)) {
            ...
            throw new IOException("Failed to encode value " + content + " as type {" + namespace +
                                  "}" + name);
        }
        return this;
    }
    ...
    public void cdsect(String text) throws IOException {
        /*
         * I'm pretty sure a CDATA section is perfectly okay to output as such in a Xebu document.
         * You shouldn't use CDATA anyway.
         */
        this.text(text);
    }
    ...
    public void comment(String text) throws IOException {
        if (text == null) { throw new IllegalArgumentException("Comment must be non-null"); }
        putAttributeEnd();
        putWaitingContent(false);
        putContentEnd();
            ...
            if (XasUtil.XSD_NAMESPACE.equals(namespace)) {
                if (name != null) {
                    if (name.equals("boolean")) {
                        ...
                        Byte b = (Byte) o;
                        if (b != null) {
                            typedData.append(new String(new byte[] { b.byteValue() }, "ISO-8859-1"));
                            result = true;
                        }
                    }
                }
