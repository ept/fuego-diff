...
/**
 * Test Xebu encoding. This class is a collection of JUnit tests for various aspects of the Xebu
 * encoding implemented in the {@link fuegocore.message.encoding} package. Generic XML serialization
 * and parsing tests can be found in the {@link fuegocore.util.tests.XasTest} class; the tests here
 * exercise some Xebu-specific functionality.
 */
public class XebuTest extends TestCase {
    ...
    protected void setUp() {
        try {
            ...
            simple.simpleElement(FOO_NS, "bar", "Non-random text", Event.createAttribute("", "baz",
                                                                                         "quux"));
            simple.simpleElement(FOO_NS, "bar", "Non-random text", Event.createAttribute("", "baz",
                                                                                         "quux"));
    ...
    /**
     * Test a simple document with repeated content. This class tests a serialize-parse cycle for a
     * simple document consisting of two identical elements in a row. This exercises the
     * tokenization and caching in Xebu.
     */
    public void testSimpleDocument() throws Exception {
        ...
        for (Iterator i = types.iterator(); i.hasNext();) {
            ...
            XebuCodecFactory factory = (XebuCodecFactory) CodecIndustry.getFactory(type);
            ...
            ser.setProperty(XebuConstants.PROPERTY_COA_MACHINE, new SimpleEoaMachine());
            ...
            par.setProperty(XebuConstants.PROPERTY_COA_MACHINE, new SimpleDoaMachine());
    ...
    /**
     * Test a document containing integer lists. This class tests a serialize-parse cycle for a
     * document containing typed content with a content codec and a COA machine registered for that
     * content. The test ensures that typed content is correctly encoded and COA machines work as
     * specified.
     */
    public void testIntListDocument() throws Exception {
        ...
        for (Iterator i = types.iterator(); i.hasNext();) {
            ...
            StringWriter target = new StringWriter();
            ser.setOutput(target);
            ContentEncoder tEnc = (ContentEncoder) ser.getProperty(XasUtil.PROPERTY_CONTENT_CODEC);
            ser.setProperty(XasUtil.PROPERTY_CONTENT_CODEC, new IntListEncoder(tEnc));
            ser.setProperty(XebuConstants.PROPERTY_COA_MACHINE, new IntListEoaMachine());
            ...
            assertNotNull("Returned parser null for type " + type, par);
            par.setInput(new StringReader(inter));
            par.setProperty(XebuConstants.PROPERTY_COA_MACHINE, new IntListDoaMachine());
            ContentDecoder tDec = (ContentDecoder) par.getProperty(XasUtil.PROPERTY_CONTENT_CODEC);
            EventSequence res = new TypedEventStream(new EventStream(par), new IntListDecoder(tDec));
            assertEquals("Enc-dec sequence failed for type " + type + "\n" + intListDocument +
                         "\n\n" + res + "\n\n", intListDocument, res);
        }
    }
    ...
    /**
     * Test the separation of consecutive content events. This tests a serialize-parse cycle for a
     * document where some content is provided as a sequence of consecutive {@link Event#CONTENT}
     * events. Since Xebu is intended to be streamable, this separation needs to persist across the
     * cycle.
     */
    public void testCoalescedContent() throws Exception {
        ...
        for (Iterator i = types.iterator(); i.hasNext();) {
            ...
            StringWriter target = new StringWriter();
            ser.setOutput(target);
            ser.setProperty(XebuConstants.PROPERTY_COA_MACHINE, new ContentEoaMachine());
            ...
            assertNotNull("Returned parser null for type " + type, par);
            par.setInput(new StringReader(inter));
            par.setProperty(XebuConstants.PROPERTY_COA_MACHINE, new ContentDoaMachine());
            EventSequence res = new EventStream(par);
            assertEquals("Enc-dec sequence failed for type " + type + "\n" + contentDocument +
                         "\n\n" + res + "\n\n", contentDocument, res);
        }
        ...
        public boolean encode(Object o, String namespace, String name, TypedXmlSerializer ser)
            ...
            if (FOO_NS.equals(namespace) && LIST_TYPE.equals(name) && o instanceof List) {
                ...
                for (int i = 0; i < l.size(); i++) {
                    writer.typedElement(FOO_NS, "item", XSD_NS, "int", l.get(i));
                }
                result = true;
            } else {
    ...
    private static class SimpleEoaMachine implements EoaMachine {
        ...
        public Event nextEvent(Event ev) {
            ...
            switch (state) {
                ...
                case 1:
                    if (ev.getType() == Event.START_ELEMENT &&
                        Util.equals(ev.getNamespace(), FOO_NS) && Util.equals(ev.getName(), "bar")) {
                        ev = null;
                        state = 2;
                    } else {
                        state = 3;
        ...
        public Event nextEvent(Event ev) {
            ...
            switch (state) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (ev.getType() == Event.ATTRIBUTE && Util.equals(ev.getNamespace(), XSI_NS) &&
                        Util.equals(ev.getName(), "type") &&
                        Util.equals(ev.getValue(), ((String) prefixMapping.get(FOO_NS)) + ":list")) {
                        ev = null;
                ...
                case 3:
                    if (ev.getType() == Event.ATTRIBUTE && Util.equals(ev.getNamespace(), XSI_NS) &&
                        Util.equals(ev.getName(), "type") &&
                        Util.equals(ev.getValue(), ((String) prefixMapping.get(XSD_NS)) + ":int")) {
                        ev = null;
                    } else if (ev.getType() == Event.END_ELEMENT &&
                               Util.equals(ev.getNamespace(), FOO_NS) &&
                               Util.equals(ev.getName(), "item")) {
    ...
    private static class IntListDoaMachine extends IdentityDoaMachine {
            ...
            switch (state) {
                case 0:
                    if (ev.getType() == Event.START_ELEMENT) {
                        ...
                        queue.enqueue(ev);
                        queue.enqueue(Event.createAttribute(XSI_NS, "type",
        ...
        public void promiseEvent(int type) {
            ...
            if (state == 2 && type == Event.TYPED_CONTENT) {
                ...
                queue.enqueue(Event.createAttribute(XSI_NS, "type",
                                                    ((String) prefixMapping.get(XSD_NS)) + ":int"));
                state = 3;
            }
        }
    ...
    private static class ContentEoaMachine implements EoaMachine {
        ...
        public Event nextEvent(Event ev) {
            switch (state) {
                ...
                case 1:
                    if (ev.getType() == Event.END_ELEMENT &&
                        Util.equals(ev.getNamespace(), FOO_NS) && Util.equals(ev.getName(), "last")) {
                        ev = null;
                        state = 0;
                    }
                    break;
        ...
        public void nextEvent(Event ev) {
            switch (state) {
                case 0:
                    queue.enqueue(ev);
                    if (ev.getType() == Event.START_ELEMENT &&
                        Util.equals(ev.getNamespace(), FOO_NS) && Util.equals(ev.getName(), "name")) {
