...
/**
 * This is a simple console application for setting up different parametrized test environments. The
 * idea is that coders implement classes derived from <code>fuegocore.util.test.AdapterClass</code>
 * and introduce these to the class list in the beginning of the source code of this class. <br/>
 * The console enables the user to create arbitrary instances of each test class. These instances
 * may then be further parametrized and configured to suit different situations. The components may
 * have arbitrary functionality. <br/>
 * The advantage over regression tests is that TestConsole provides fast and interactive test
 * reconfigurability. It can also be used to troubleshoot problems revealed by tests, particularily
 * if the components support some kind of reflectivity. <br/>
 * The console can be run interactively or passing a ready-made batch of commands to the stdin. <br>
...
public class TestConsole {
    ...
    static {
        ...
        KNOWN_CLASSES.put("PrTrans",
                          new TestClass("PrTrans",
    ...
    /**
     * How long each instruction is allowed to execute before it is considered blocked. The
     * execution is then abandoned (not killed) and the control retuns to the UI.
     */
    ...
    private void mainLoop(BufferedReader reader, BufferedWriter writer, String initParameter) {
    ...
    public String exec(String line) {
        ...
        if (!alpha.matcher(command).matches()) { return "** Malformed variable name\n# "; }

        AdapterClass adapter = (AdapterClass) this.variables.get(command);
    ...
    static final class Executor extends Thread {
        ...
        public void run() {
            this.returnee = this.console.exec(command);
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            parameter = parameter == null ? args[i] : parameter + " " + args[i];
        }
    ...
    public static abstract class AdapterClass {
        ...
        /**
         * General "constructor". All AdapterClasses are instantiated using a construct with no
         * arguments. This method will be called right after instantiation.
         * @param console
         *            A reference to the instantiating console, is stored to fiel
         *            <code>console</code>
         * @param parameter
         *            An arbitrary user-given string, not stored
         * @param name
         *            The UI class name assigned for this Java Class, stored in the field
         *            <code>name</name>
         * @return String An arbitrary message that is printed for the user. <code>null</code>
         *         should be interpreted as a failure.
         */
        public String create(TestConsole console, String parameter, String name) {
            this.name = name;
            this.console = console;
            return SUCCESS;
        }
         ...
         * @param parameter
         *            An arbitrary parameter given by the user
         ...
         * @param parameter
         *            Any parameters the user has given.
        ...
        /**
         * Get help on the usage of this instance. This text may well vary depending on the current
        ...
        /**
         * Request that if the test adapter has any logging to do, it will do it to the provided
         * <code>java.util.logging.Logger</code> instance. The default implementation just stores
         * the reference in the field <code>log</code>.
         * @param log
         *            The reference *DUH*
         */
        ...
        /**
         * This requests that if the test has any significant amount of data to write, it should be
         * written to the given Writer. Especially useful for backgroup operation. The default
         * implementation just stores the the Writer reference to field <code>writer</code>.
         * @param writer
         *            The java.io.Writer reference
         */
    ...
    /**
     * The default class to handle the basic UI functionality. Namely instantiating new classes and
    ...
    public static final class RootClass extends AdapterClass {
        ...
        public String invoke(String parameter) {
            if ("?".equals(parameter)) {
                ...
                AdapterClass variable = (AdapterClass) console.variables.get(parameter);
                return (variable != null) ? variable.getHelp() : "** ERROR no such variable";
            } else if (parameter.startsWith("new ")) {
                ...
                if (parameter.length() < 1) { return "** ERROR Invalid variable name"; }
                ...
                try {
                    adapter = (AdapterClass) Class.forName(aClass.test).newInstance();
                } catch (InstantiationException ex) {
                    log.log(Level.WARNING, "Error instantiating " + aClass.test, ex);
                    return "** ERROR: " + ex.getMessage();
                } catch (IllegalAccessException ex) {
                    log.log(Level.WARNING, "Error instantiating " + aClass.test, ex);
                    return "** ERROR: " + ex.getMessage();
                } catch (ClassNotFoundException ex) {
                    log.log(Level.WARNING, "Adapter class not found: " + aClass.test, ex);
                    return "** ERROR: Adapter class not available";
                }

                String returnee = adapter.create(this.console, parameter, aClass.name);
                if (returnee != null) {
                ...
                AdapterClass removable = (AdapterClass) console.variables.remove(parameter);
                if (removable != null) {
                    return removable.destroy(option);
                } else return "** ERROR no such variable";
            } else if ("list".equals(parameter)) {
                StringBuffer returnee = new StringBuffer();
                Iterator cIter = KNOWN_CLASSES.values().iterator();
                Iterator vIter = console.variables.entrySet().iterator();
                ...
                returnee.append("\n** Known variables:");
                while (vIter.hasNext()) {
                    java.util.Map.Entry entry = (java.util.Map.Entry) vIter.next();
                    returnee.append(" ").append(entry.getKey()).append("(").append(
                                                                                   ((AdapterClass) entry.getValue()).getName()).append(
                                                                                                                                       ")");
                }
                return returnee.toString();
            } else if (parameter.startsWith("write ")) {
                parameter = parameter.substring(6);
                int space = parameter.indexOf(' ');
                if (space < 1) { return "** ERROR missing filename"; }
                String filename = parameter.substring(space + 1);
                parameter = parameter.substring(0, space);
                AdapterClass adapter = (AdapterClass) this.console.variables.get(parameter);
                ...
                return "** Output from " + parameter + " requested to " + filename;
            } else if (parameter.startsWith("wait ")) {
        ...
        public String getHelp() {
            return "?\tThis help\n" + "? NAME\tCommands that a particular object accepts\n"
                   ...
                   + "write NAME FILE\tDirect data output from object NAME to " + "file FILE\n"
    ...
    /**
     * A core supporting Class that allows grouping a set of commands to a set that can be invoked
     * together either at the same time (paraller) of as a batch (serial). When firing the execution
     * this class will return only after all invocations are complete and then return the results of
     * each at the same time, in the order they were given.
     */
    public static final class TriggerClass extends AdapterClass {
        ...
        public String invoke(String parameter) {
            if ("serial".equals(parameter)) {
                this.serial = true;
                return "** SERIAL MODE";
            } else if ("paraller".equals(parameter)) {
                this.serial = false;
                return "** PARALLER MODE";
            } else if ("list".equals(parameter)) {
                StringBuffer returnee = new StringBuffer();
                returnee.append("Current execution queue").append(
                                                                  this.serial ? "(serial)"
                                                                          : "(paraller)");
                ...
                return returnee.toString();
            } else if (parameter.startsWith("import ")) {
        ...
        public String getHelp() {
            return "Triggers several object invocations at one time"
        ...
        private String fireSerial() {
            Iterator iter = this.invocations.iterator();
            StringBuffer returnee = new StringBuffer();
            returnee.append("Executing queue of ").append(this.invocations.size()).append(
                                                                                          " commands");
            while (iter.hasNext()) {
                String next = iter.next().toString();
                returnee.append("\n>> executing: " + next).append("\n").append(
                                                                               this.console.exec(next));
            }
            return returnee.toString();
        }
        ...
        private String fireParaller() {
            ...
            while (iter.hasNext()) {
                ...
                returnee.append("\n>> execution: ").append(thread.command).append(
                                                                                  thread.getResult());
            }
    ...
    /**
     * This adapter can configure the Java2 logging facility, redirecting loggers to files or
     * stdout.
     */
    public static final class LogAdapter extends AdapterClass {
        ...
        public String destroy(String parameter) {
            if (this.logHandler instanceof SocketHandler || this.logHandler instanceof FileHandler) {
        ...
        public String getHelp() {
            return "list\tList all currently known Loggers\n" +
                   "out -|FILE|host:port\tSet the output of this Logger\n" +
                   "attach name\tAttach this Logger for the given object\n" +
                   "level LEVEL\tSet the log level, either name or number " + "[0-1000]\n" +
        ...
        public String invoke(String parameter) {
            assert parameter != null && parameter.length() > 0 : "Invocation with empty parameter is illegal";
            ...
            if ("list".equals(parameter)) {
                ...
                if (this.myLogger == null) { return "** ERROR no logger associated"; }
                ...
                if ("-".equals(parameter)) {
                    ...
                    if (index >= parameter.length()) { return "** ERROR port must be defined"; }
                    String host = parameter.substring(0, index);
                    String port = parameter.substring(index + 1);
                    try {
                        newHandler = new SocketHandler(host, Integer.parseInt(port));
                        this.thisOutput = "network: " + parameter;
                    } catch (NumberFormatException ex) {
                        return "** ERROR port must be numeric: " + port;
                    } catch (IllegalArgumentException ex) {
                        return "** ERROR invalid host or port: " + parameter + "(" +
                               ex.getMessage() + ")";
                    } catch (IOException ex) {
                        return "** ERROR can not connect: " + parameter + "(" + ex.getMessage() +
                               ")";
                    }
                } else {
                    try {
                        ...
                        return "** ERROR opening file " + parameter + "(" + ex.getMessage() + ")";
            ...
            } else if (parameter.startsWith("attach ")) {
                if (this.myLogger == null) { return "** ERROR no logger associated"; }

                parameter = parameter.substring(7);
                AdapterClass object = (AdapterClass) this.console.variables.get(parameter);
                if (object != null) {
            ...
            } else if (parameter.startsWith("level ")) {
                if (this.myLogger == null) { return "** ERROR no logger associated"; }
