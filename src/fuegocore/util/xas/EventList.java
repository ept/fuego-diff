...
/**
 * A {@link Vector}-based representation of XML event sequences. This class implements the
 * construction of XML event sequences by the use of methods similar in signature to those in the
 * {@link Vector} interface. The main difference is that the methods in this class only take
 * {@link Event} objects as arguments instead of arbitrary objects.
 * <p>
 * The invariants that the modification methods in this class are intended to guarantee are
 * <ul>
 * <li>All events of type {@link Event#ATTRIBUTE} are immediately preceded by either an
 * {@link Event#ATTRIBUTE} event or an {@link Event#START_ELEMENT} event</li>
 * <li>Events of type {@link Event#CONTENT} are neither preceded nor succeeded by events of the same
 * type</li>
 * </ul>
 */
public class EventList implements EventSequence {
    ...
    private boolean coalesceNext(int index) {
        ...
        if (index < current.size() - 1) {
            ...
            if (curr.getType() == Event.CONTENT && next.getType() == Event.CONTENT) {
    ...
    /**
     * Add an event to the sequence. This method adds the given event to the end of the sequence,
     * verifying that it is allowed and coalescing adjacent {@link Event#CONTENT} events.
     * @param ev
     *            the event to add
     * @throws IllegalArgumentException
     *             if the given event is not allowed at this point
     */
    ...
    /**
     * Add an event to the sequence at the specified place. This method adds the given event to have
     * the specified index in the sequence, shifting subsequent events, if any, to the right. The
     * method verifies that addition is allowed and coalesces adjacent {@link Event#CONTENT} events.
     * @param ev
     *            the event to add
     * @throws IllegalArgumentException
     *             if the given event is not allowed at this point
     * @throws IndexOutOfBoundsException
     *             if the index is out of range (smaller than <code>0</code> or larger than the
     *             current size of the sequence)
     */
    public void add(int index, Event ev) {
        int i = toPrivate(index);
        if (i < 0 || i > current.size()) { throw new IndexOutOfBoundsException("Index " + index +
                                                                               " out of range"); }
        if (ev.getType() == Event.ATTRIBUTE) {
            if (i > 0) {
                Event prev = (Event) current.elementAt(i - 1);
                int prevType = prev.getType();
                if (prevType != Event.START_ELEMENT && prevType != Event.ATTRIBUTE) { throw new IllegalArgumentException(
                                                                                                                         "ATTR not allowed " +
                                                                                                                                 "after type " +
                                                                                                                                 prevType); }
            }
    ...
    /**
     * Add all events from another event sequence to the end of this sequence. The events are added
     * as if the sequence was iterated through and each individual event was added with
     * {@link #add(Event)}.
     * @param es
     *            the event sequence to append
     */
    ...
    /**
     * Remove an event at the specified index. This method removes the event at the given index from
     * the event sequence.
     * @param index
    ...
    public EventSequence subSequence(int from, int to) {
        ...
        int t = toPrivate(to);
        if (f < 0 || t > current.size()) { throw new IllegalArgumentException(
                                                                              "Out of range: from=" +
                                                                                      from +
                                                                                      ", to=" + to); }
    ...
    public Enumeration events() {
        return new ListEnumerator(this, getSmallestActiveIndex(), getLargestActiveIndex() + 1);
    }
     ...
     * class <code>EventList</code>; it is sufficent that it implement the {@link EventSequence}
    ...
    private class SubSequence implements EventSequence {
        ...
        public EventSequence subSequence(int from, int to) {
            if (!check(from) || !check(to - 1)) { throw new IllegalArgumentException(
                                                                                     "Out of range: from=" +
                                                                                             from +
                                                                                             ",to=" +
                                                                                             to); }
