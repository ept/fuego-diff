...
/**
 * A class for creating an {@link EventSequence} from serialization. This class builds an event
 * sequence by accepting calls on the {@link TypedXmlSerializer} interface, adding events for calls
 * that would add events to the output stream. The main purpose of this is to permit building an
 * abstract representation of the XML document from a legacy application that expects to be
...
public class EventSerializer implements TypedXmlSerializer {
    ...
    /**
     * Return the collected sequence. This method returns the sequence of events that have so far
     * been collected. It is returned as read-only, since it is not intended for modification by
     * applications.
     */
    ...
    /**
     * A do-nothing method. Since the events are inserted into the event sequence, setting an output
     * stream is meaningless. However, this method must not throw an exception, since an application
     * will expect a successful call to this before outputting events.
     */
    ...
    public XmlSerializer endTag(String namespace, String name) {
        if (depth <= 0) { throw new IllegalStateException("End tag {" + namespace + "}" + name +
                                                          " without start tag"); }
        depth -= 1;
