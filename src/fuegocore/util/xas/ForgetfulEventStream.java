...
/**
 * An event sequence with constant space use. This class wraps an existing {@link EventSequence} and
 * keeps track of how many events are in the wrapped sequence. When the size of the wrapped sequence
 * grows beyond a specified threshold, the sequence's {@link EventSequence#forgetUntil} method is
 * automatically called to bring its size back to a low value.
 * <p>
 * At construction time, both the minimum and maximum values for the number of events are given.
 * Whenever the number of events exceeds the specified maximum value, it is brought back to the
...
public class ForgetfulEventStream implements EventSequence {
    ...
    /**
     * The default constructor. This constructor sets the minimum and maximum sizes to their default
     * values.
     * @param es
    ...
    /**
     * The threshold constructor. This constructor sets the minimum and maximum sizes to the
     * specified values.
     * @param es
     *            the event sequence to wrap
     * @param low
     *            the minimum number of events permitted
     * @param high
     *            the maximum number of events permitted
     */
    public ForgetfulEventStream(EventSequence es, int low, int high) {
        if (low > high) { throw new IllegalArgumentException("Must have low <= high, got " +
                                                             "low=" + low + ",high=" + high); }
        this.es = es;
        this.low = low;
        this.high = high;
