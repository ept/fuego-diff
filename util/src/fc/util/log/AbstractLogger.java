...
/**
 * A helper class for implementing loggers. This class provides a generic
 * foundation that can be used by several loggers. It implements the level
 * checks of {@link Logger#isEnabled} and the {@link Logger#log(Object,int)} as
 * a convenience.
 * 
...
public abstract class AbstractLogger implements Logger {
    ...
    /**
         * The name of the system property for setting the log level. This is
         * marked {@code public} so that Javadoc can find it in {@code @value}
         * statements.
         */
    ...
    /**
         ...
         * {@link LogLevels#INFO}, but can be reset in a subclass constructor
         * or using the system property {@value #PROPERTY_LEVEL}; the values
         * for this property are given by each log level in {@link LogLevels}.
         * It may be freely changed at run time with the changes being visible
         * in the object's behavior.
         */
    ...
    /**
         * Return the method name that called logging. This method will trawl
         * through the current stack trace and pick the first one the is not in
	...
	for (StackTraceElement f : stack) {
	    try {
		if (!f.getClassName().startsWith(
		    AbstractLogger.class.getPackage().getName())
			&& !Logger.class.isAssignableFrom(Class.forName(f
			    .getClassName()))) {
		    frame = f;
		    break;
		}
