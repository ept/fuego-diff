 ...
 * interface is {@link TypedXmlParser}.  The parser implements all
 * caching features of the Xebu format.
 *
...
public class XebuParser extends ContentDecoder implements TypedXmlParser {
	...
	return cachedEvent != null
	    || (cachedSequence != null && cachedSequence.hasMoreElements());
    }
	...
	if (cachedEvent != null) {
	    result = cachedEvent;
	    cachedEvent = null;
	} else if (cachedSequence != null
		   && cachedSequence.hasMoreElements()) {
	...
	if ((flags & XebuConstants.VALUE_FETCH) == XebuConstants.VALUE_FETCH) {
	    ...
	    if (ev == null) {
		throw new XmlPullParserException("No value cached at index "
						 + index, this, null);
	    }
	    ...
	    if (ev == null) {
		throw new XmlPullParserException("No name cached at index "
						 + index, this, null);
	    }
	    ...
	    if (ans == null) {
		throw new XmlPullParserException("No namespace cached at "
						 + "index " + index, this,
						 null);
	    }
    ...
    private Event advanceAttribute ()
	throws IOException, XmlPullParserException {
	Event result = null;
	boolean ready = false;
	...
	while (!ready) {
		...
		if ((c & XebuConstants.TOKEN_SPACE)
    ...
    private int advancePreparedEvent (Event ev)
	throws IOException, XmlPullParserException {
	int result = -1;
	...
	case Event.START_ELEMENT:
		...
		insertAttribute(ev.getNamespace(), ev.getName(),
				(String) ev.getValue());
	    }
	    result = START_TAG;
	    break;
    ...
    private void advanceReaderEvent ()
	throws IOException, XmlPullParserException {
	Event doaEvent = null;
	int c = readToken();
	if (c == -1) {
		...
		cacheSequence =
		    (flags & XebuConstants.FLAG_SEQUENCE_CACHING) != 0;
		...
		doaEvent =
		    Event.createEndElement(elementStack[2 * (depth - 1)],
		...
		if ((flags & XebuConstants.COALESCE_FLAG)
		...
		if ((flags & XebuConstants.VALUE_FETCH)
		    ...
		    int index = readToken();
		    typedData =
			(String) caches[XebuConstants.CONTENT_INDEX][index];
		} else if ((flags & XebuConstants.TYPED_MULTIDATA_FLAG)
			   == XebuConstants.TYPED_MULTIDATA_FLAG) {
		...
		if (!cacheSequence) {
		    throw new XmlPullParserException("Sequence caching "
						     + "operation not "
						     + "permitted for this "
						     + "document", this, null);
		}
		...
		if (index < 0 || index >= sequenceCache.size()) {
		    throw new XmlPullParserException("Invalid sequence cache "
						     + "index: " + index,
						     this, null);
		}
		EventSequence es =
		    (EventSequence) sequenceCache.elementAt(index);
		...
		if (seqIndex < 0) {
		    throw new XmlPullParserException("Invalid cached sequence "
						     + "length: " + length
						     + ", should have been "
						     + "less than "
		...
		sequenceCache.setElementAt(currentSequence.subSequence(seqIndex),
					   index);
		break;
	    }
    ...
    private int advanceContentFromStream ()
	throws IOException, XmlPullParserException {
	int result = -1;
	int c = readToken();
	if (c < 0) {
	    throw new XmlPullParserException("Unexpected end of stream", this,
					     null);
	}
	int token = tokenize(c);
	int flags = flagize(c);
	if (token == XebuConstants.DATA_START) {
		...
		currentData =
		    (String) caches[XebuConstants.CONTENT_INDEX][index];
		if (currentData == null) {
		    throw new XmlPullParserException("No content cached at "
						     + "index " + index,
						     this, null);
		}
	    } else {
	    ...
	    if (token != XebuConstants.ATTRIBUTE_END) {
		throw new XmlPullParserException("Content sequence not ended "
						 + "by end token", this, null);
	    }
	...
	while (type == -1) {
		...
		if (this.namespace == null || this.name == null) {
		    ...
		    doa.nextEvent(Event.createTypedContent(this.namespace,
							   this.name, object));
		    areDecoded = true;
		}
	    } else {
	...
	if (XebuConstants.PROPERTY_INITIAL_CACHES.equals(name)) {
	    ...
	    checkInProgress();
	    if (!(value instanceof DoaMachine)) {
		throw new IllegalArgumentException("Value " + value + " is "
						   + "not a valid COA "
						   + "machine");
	    }
