...
/**
 * Common XML documents for testing.  This class creates a group of
 * {@link EventSequence} objects representing different XML documents.
 * These can be used to test various XML handling classes.
 */
public class XasData {
    ...
    /**
     * Get all XML documents.  This method returns the XML documents
     * that this class has created.
     *
     * @return XML documents for testing as {@link EventSequence}
     * objects
     */
	...
	try {
	    ...
	    zamunda.simpleElement(FOO_NS, "ruler", "Jaffe Joffer",
				  Event.createAttribute(FOO_NS, "title",
							"King"));
	    zamunda.simpleElement(FOO_NS, "ruler", "Jaffe Joffer",
				  Event.createAttribute(FOO_NS, "title",
							"King"));
	    zamunda.simpleElement(FOO_NS, "futureruler", "Akeem",
				  Event.createAttribute(FOO_NS, "title",
							"Prince"));
	    zamunda.simpleElement(FOO_NS, "futureruler", "Akeem",
				  Event.createAttribute(FOO_NS, "title",
	    ...
	    list.simpleElement(FOO_NS, "ruler", "Jaffe Joffer",
			       Event.createAttribute(FOO_NS, "title", "King"));
		...
		new XmlWriter(c).complexElement(FOO_NS, "content", content,
						Event.createAttribute(FOO_NS, "height", Integer.toString(i)));
	    ...
	    typed.addEvent(Event.createNamespacePrefix(XasUtil.XSI_NAMESPACE,
						       "xsi"));
	    typed.addEvent(Event.createNamespacePrefix(XasUtil.XSD_NAMESPACE,
						       "xsd"));
	    typed.typedElement(FOO_NS, "integer", XasUtil.XSD_NAMESPACE, "int",
			       new Integer(500));
	    ...
	    compound.typedElement(FOO_NS, "compound", FOO_NS, CP_TYPE,
				  new Compound(500, (byte) 50, bs, c));
	    ...
	    intList.addEvent(Event.createNamespacePrefix(XasUtil.XSI_NAMESPACE,
							 "xsi"));
	    intList.addEvent(Event.createNamespacePrefix(XasUtil.XSD_NAMESPACE,
							 "xsd"));
    ...
    /**
     * A compound object with a few fields of different types.  This
     * is basically a record with some differently-typed fields for
     * testing typed content.
     */
	...
	public boolean equals (Object o) {
	    ...
	    return i == cp.i && b == cp.b && Arrays.equals(bs, cp.bs)
		&& Util.equals(c, cp.c);
	}
	...
	public boolean encode (Object o, String namespace, String name,
	    ...
	    if (FOO_NS.equals(namespace) && CP_TYPE.equals(name)
		&& o instanceof Compound) {
		Compound cp = (Compound) o;
		String prefix = ser.getPrefix(namespace, false);
		ser.attribute(XasUtil.XSI_NAMESPACE, "type",
			      prefix + ":" + name);
		XmlWriter writer = new XmlWriter(ser);
		writer.typedElement("", "i", XasUtil.XSD_NAMESPACE, "int",
				    new Integer(cp.getI()));
		writer.typedElement("", "b", XasUtil.XSD_NAMESPACE, "byte",
				    new Byte(cp.getB()));
		writer.typedElement("", "bs", XasUtil.XSD_NAMESPACE,
				    "base64Binary", cp.getBs());
		writer.typedElement("", "c", XasUtil.XSD_NAMESPACE,
				    "dateTime", cp.getC());
		result = true;
	    } else if (chain != null) {
	...
	public Object decode (String typeNs, String typeName,
		...
		if (io != null && bo != null && bs != null && c != null) {
		    result = new Compound(io.intValue(), bo.byteValue(), bs,
					  c);
		} else {
    ...
    /**
     * A {@link ContentEncoder} for an integer list.  The object is of
     * type {@link List} with all elements of type {@link Integer}.
     */
	...
	public boolean encode (Object o, String namespace, String name,
		...
		for (int i = l.size() - 1; i >= 0; i--) {
		    writer.typedElement(FOO_NS, "item", XasUtil.XSD_NAMESPACE,
					"int", l.get(i));
		}
		result = true;
	    } else {
    ...
    /**
     * A {@link ContentDecoder} for an integer list.  The object is of
     * type {@link List} with all elements of type {@link Integer}.
     */
