...
/**
 * An XML sequence that is incrementally compiled from a stream.  This
 * class implements the construction of XML event sequences by using a
 * pull parser to read XML events as they are requested by callers of
 * its methods.  This makes it possible to process XML documents in a
 * streaming fashion, i.e. process parts before the whole is even
 * available.
 *
 * <p>The only way to get events read from the stream is to request
 * them.  If there is a need to get every event out from the stream,
 * the {@link #events} method should be used to get an {@link
 * Enumeration} and iterate through the events.
 */
public class EventStream implements EventSequence {
	...
	if (inProgress) {
		...
		case TypedXmlParser.START_TAG: {
		    int nsStart =
			source.getNamespaceCount(source.getDepth() - 1);
		...
		case TypedXmlParser.OBJECT:
		    current.addElement
			(Event.createTypedContent(source.getNamespace(),
						  source.getName(),
						  source.getObject()));
		    break;
		...
		ex.printStackTrace();
		/*
		 * Just set inProgress to be false, since the document
		 * parsed so far may be of interest anyway.
		 */
		inProgress = false;
	    }
	}
	...
	while (inProgress && (index < 0
			      || current.size() <= toPrivate(index))) {
	    getNextEvent();
	}
    ...
    /**
     * Return the event at the specified point in the sequence.  If
     * the sequence does not yet contain enough events and the stream
     * has not been exhausted, this method will read additional events
     * from the stream until either the requested event is available
     * or the stream has been exhausted.
     *
     ...
     * ending index.  If there are not enough events already read and
     * the stream has not been exhausted, this method will read
     * additional events from the stream until either enough events
    ...
    /**
     * Return an iterator over the events in this sequence.  All
     * objects returned by the iterator will be of type {@link Event}.
     * The returned iterator will cause additional events to be read
     * from the stream into the sequence automatically as they are
     * requested.
     */
     ...
     * argument object need not be of class <code>EventStream</code>;
     * it is sufficent that it implement the {@link EventSequence}
    ...
    /**
     * Return a string representation of this object.  The string
     * returned by this method will contain all events that are
     * logically part of the stream, so this method will read events
     * from the stream until the stream is exhausted.  This method
     * should therefore be used only in debugging code.
     *
