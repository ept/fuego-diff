...
/**
 * General utilities often needed in connection with the XAS API.
 * This class collects (as static methods) several utility methods
 * that are typically needed by applications using or extending the
 * XAS API.
 *
 * <p>The difference between this class and the {@link XasUtil} class
 * is that this class is allowed to contain methods depending on
 * libraries outside the MIDP and CLDC specifications.  This
...
public class XasExtUtil {
    ...
    private static Node readerToDom (Document doc, XmlReader xr,
				     Stack nss) {
	Node result = null;
	...
	case Event.NAMESPACE_PREFIX:
	    do {
		pms.add(xr.advance());
	    } while ((ev = xr.getCurrentEvent()).getType()
	...
	case Event.ENTITY_REFERENCE:
	    ...
	    break;
	default:
	    if ((ev.getType() & Event.FLAG_BITMASK)
		== Event.TYPE_EXTENSION_FLAG) {
    ...
    /**
     * Convert XML from an XML reader to a DOM tree.  This method
     * takes a {@link XmlReader} and returns a {@link Node}
     * representing the same XML document or a fragment of it.  The
     * XML represented by the underlying event sequence of the reader
     * needs to be well-formed and balanced, and must not contain
     * {@link Event#TYPED_CONTENT} events.  A DOM {@link Document}
     * object needs to be provided to act as a factory for DOM {@link
     * Node} objects.
     *
     * <p>If the underlying event sequence of the reader represents a
     * complete XML document, the passed in DOM {@link Document} needs
     * to be empty, since the resulting document will be built into
     * that.
     *
     * @param doc the DOM document to use for creating DOM nodes
     * @param xr the reader from which to read the converted event
     * sequence
     * @return the DOM node representing <code>es</code>
     *
     * @throws IllegalArgumentException if the XML represented by
     * <code>xr</code>'s underlying event sequence is not convertible
     * to DOM
     */
    ...
    /**
     * Convert XML from an event sequence to a DOM tree.  This method
     * takes a {@link EventSequence} and returns a {@link Node}
     * representing the same XML document or a fragment of it.  The
     * XML represented by the event sequence needs to be well-formed
     * and balanced, and must not contain {@link Event#TYPED_CONTENT}
     * events.  A DOM {@link Document} object needs to be provided to
     * act as a factory for DOM {@link Node} objects.
     *
     * <p>If the event sequence represents a complete XML document,
     * the passed in DOM {@link Document} needs to be empty, since the
     * resulting document will be built into that.
     *
     * @param doc the DOM document to use for creating DOM nodes
    ...
    /**
     * Convert XML from a DOM tree to an event sequence.  This method
     * takes a DOM tree as a {@link Node} and returns a {@link
     * EventSequence} representing the same XML document or a fragment
     * of it.  Some DOM features (such as document types) are not
     * supported by this method, since {@link EventSequence} does not
     * support them.
     *
	...
	case Node.ELEMENT_NODE: {
		...
		if (name.startsWith("xmlns:")) {
		    ...
		    atts.add(Event.createAttribute(att.getNamespaceURI(),
						   att.getLocalName(),
						   att.getValue()));
		}
	    }
	    el.addAll(pms);
	    el.add(Event.createStartElement(n.getNamespaceURI(),
					    n.getLocalName()));
	    el.addAll(atts);
	    ...
	    el.add(Event.createEndElement(n.getNamespaceURI(),
					  n.getLocalName()));
	    break;
	}
	case Node.TEXT_NODE:
    ...
    /** Get an XML parser suitable for the given input stream. Uses
     * a {@link XasExtUtil.XebuXmlIdentifier} to determine the mime type.
     * @param in input stream that will be attached to the parser
     * @param enc input character encoding, e.g. <code>utf-8</code>
     * @return a parser suitable for the input stream,
     * or null if none available
     * @throws IOException if an I/O error occured when determining the
     * type of the stream.
     */
    ...
    public static TypedXmlParser getParser(MagicInputStream in,
                                           String enc) throws
        IOException {
    ...
    public static TypedXmlSerializer getSerializer(OutputStream out,
        ...
        if( !isBinary )
         ser.setFeature("http://xmlpull.org/v1/doc/features.html#indent-output",
                        true);
        return ser;
    }

    /** Identifies textual and Xebu encoded XML files.
     * Identification is done by inspecting the first 4 bytes of the file.
     * The case insensitive string <code>&lt;?xm</code> identifies the file as
     * <code>text/xml</code>. A Xebu <code>START_DOCUMENT</code> event
     * identifies the content as Xebu encoded; the correct subtype is determined
     * from the document start flags.
     */
    ...
    public static class XebuXmlIdentifier extends MagicInputStream {
        ...
        protected String identify(byte[] magic, int len) {
            ...
            if (len < 1)
                return null;
            if ( (magic[0] & XebuConstants.TOKEN_SPACE) ==
                XebuConstants.DOCUMENT) {
                int subtype =
                   ( (magic[0] & XebuConstants.FLAG_SEQUENCE_CACHING) != 0 ? 4 :
                     0) +
                    ( (magic[0] & XebuConstants.FLAG_CONTENT_CACHING) != 0 ? 2 :
                     0) +
                    ((magic[0] & XebuConstants.FLAG_ITEM_CACHING) != 0 ? 1 : 0);
                return SUBTYPES[subtype] == null ? null :
                    "application/x-ebu+" + SUBTYPES[subtype];
            }
            return null;
        }
